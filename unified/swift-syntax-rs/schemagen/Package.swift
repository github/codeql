...
// schema describing the shape of the trees `swift-syntax-parse` emits. Run it
// through `unified/scripts/regenerate-node-types.sh`, which stages the sources
// this package needs; see `README.md` for the details.
//
...
// Swift 6 strict concurrency — its node tables are non-`Sendable` globals.
let package = Package(
    name: "schemagen",
    platforms: [
    ...
    dependencies: [
        ...
        // package resolved, rather than a second `url`/`exact:` pin: the schema
        // has to describe the very swift-syntax that `swift-syntax-parse`
        // links, and a single pin cannot drift from itself.
