// generated by codegen/codegen.py
private import codeql.swift.generated.Synth
private import codeql.swift.generated.Db
import codeql.swift.elements.decl.Decl
import codeql.swift.elements.expr.Expr

class LookupExprBase extends Synth::TLookupExpr, Expr {
  Expr getImmediateBaseExpr() {
    result =
      Synth::convertExprFromDb(Synth::convertLookupExprToDb(this).(Db::LookupExpr).getBaseExpr())
  }

  final Expr getBaseExpr() { result = getImmediateBaseExpr().resolve() }

  Decl getImmediateMember() {
    result =
      Synth::convertDeclFromDb(Synth::convertLookupExprToDb(this).(Db::LookupExpr).getMember())
  }

  final Decl getMember() { result = getImmediateMember().resolve() }

  final predicate hasMember() { exists(getMember()) }
}
