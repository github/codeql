...
void g()
{
    ...
    printf("%zu", ul); // $ Alert // not ok
    printf("%zu", st); // ok
    printf("%zu", ST); // ok
    printf("%zu", c_st); // ok
    printf("%zu", C_ST); // ok
    printf("%zu", sizeof(ul)); // ok
    printf("%zu", sst); // $ Alert // not ok

    printf("%zd", ul); // not ok [NOT DETECTED]
    printf("%zd", st); // $ Alert // not ok
    printf("%zd", ST); // $ Alert // not ok
    printf("%zd", c_st); // $ Alert // not ok
    printf("%zd", C_ST); // $ Alert // not ok
    printf("%zd", sizeof(ul)); // $ Alert // not ok
    printf("%zd", sst); // ok
    {
        char *ptr_a, *ptr_b;
        char buf[100];
        ...
        printf("%zd", ptr_a - ptr_b); // $ Alert // ok (dubious) [FALSE POSITIVE]
    }
}
...
void fun2() {
  ...
  printf("%S", myString3); // $ Alert // BAD
  printf("%S", myString4); // $ Alert // BAD
}
