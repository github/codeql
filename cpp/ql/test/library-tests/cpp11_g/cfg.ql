...
string name(Function f) {
  if f.isConstructedFrom(_) then
    result = f.toString() + "<" + max(arguments(f, _)).suffix(1) + ">"
  else if f instanceof TemplateFunction then
    result = f.toString() + " prototype instantiation"
  else
    result = f.toString()
}
...
select name(x.getControlFlowScope()),
