...
predicate dominatedByStart(Function func, ControlFlowNode node) {
  ...
  or exists (ControlFlowNode dom |
    dominatedByStart(func, dom)
    and iDominates(dom, node))
}


from Function func, ControlFlowNode entry, ControlFlowNode node
where
  func.getEntryPoint() = entry
  and reachableIn(func) = node
  and entry != node
  and not dominatedByStart(func, node)
select func, node
