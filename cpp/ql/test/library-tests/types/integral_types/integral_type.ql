...
from IntegralType t, string tStr, string signed, string unsigned,
     string explicitlySigned, string explicitlyUnsigned,
     string implicitlySigned
where tStr = (t.toString() + "                  ").prefix(18)
  and if t.isSigned()             then signed = "signed"
                                  else signed = "------"
  and if t.isUnsigned()           then unsigned = "unsigned"
                                  else unsigned = "--------"
  and if t.isExplicitlySigned()   then explicitlySigned = "explicitlySigned"
                                  else explicitlySigned = "----------------"
  and if t.isExplicitlyUnsigned() then explicitlyUnsigned = "explicitlyUnsigned"
                                  else explicitlyUnsigned = "------------------"
  and if t.isImplicitlySigned()   then implicitlySigned = "implicitlySigned"
                                  else implicitlySigned = "----------------"
select integralTypeKind(t), tStr, signed, unsigned,
       explicitlySigned, explicitlyUnsigned,
