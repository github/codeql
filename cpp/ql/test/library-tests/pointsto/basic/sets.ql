...
newtype TElementOrSet = MkElement(Element e) or MkSet(int i) { pointstosets(i, _) }
...
predicate isPointsToSetDest(boolean isEdge, Element x, Element y, string label) {
    exists(string loc, string name |
       ...
       and (if exists(x.toString())
            then name = x.toString()
            else name = "<no toString>")
       and label = loc + "\n" + name)
}
...
where isSetFlowEnd(isEdge, x.asSet(), y.asSet(), label)
   or isSetFlow(isEdge, x.asSet(), y.asSet(), label)
   or isPointsToSetSrc(isEdge, x.asSet(), y.asSet(), label)
   or isPointsToSetDest(isEdge, x.asElement(), y.asElement(), label)
   or isPointsToSets(isEdge, x.asSet(), y.asElement(), label)
