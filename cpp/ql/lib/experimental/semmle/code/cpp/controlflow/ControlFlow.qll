...
module ControlFlow {
  ...
  signature module ConfigSig {
    ...
    predicate callLabel(CallNode c, Label l, Position p);

    predicate callableLabel(Callable c, Label l, Position p);
  }
  ...
  module Configged<ConfigSig ConfigMod> {
    ...
    private predicate callTargetUniq(Callable c1, Label l1, CallNode call, Callable c2, Label l2) {
      exists(Position p |
        ...
        callLabel(call, l1, p) and
        callableLabel(c2, l2, p)
      )
    }
    ...
    private newtype TPathNode =
      TPathNodeMk(Node n, Label l) { flowFwd2(n, l) }
    ...
    class PathNode extends TPathNode {
      ...
      predicate isSource() { isSource(n, l) }

      predicate isSink() { isSink(n, l) }
    }
    ...
    predicate hasFlow(PathNode src, PathNode sink) {
      src.isSource() and
      sink.isSink() and
      src.getASuccessor+() = sink
    }
  }
}
