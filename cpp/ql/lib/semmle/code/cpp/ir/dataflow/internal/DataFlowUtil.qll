...
private Expr getConvertedResultExpression0(Instruction instr) {
  exists(ConvertInstruction convert | instr = convert |
    ...
    // int* p = new int[size]
    // ```
    // The IR translation looks like
    // ```
    // r1 = VariableAddress[size]
    // r2 = Load r1, m
    // r3 = Convert r2
    // ...
    //```
    // The result of `r2.getConvertedResultExpression()` is `size` (since r3 is actually
    // generated by the IR translation of `new int`), but since we require that only the
    // _fully converted_ instructions are mapped to expressions, we need `r3` to be the
