/**
 * Library for parsing C++ regular expressions (ECMAScript dialect, the default
 * for `std::regex`).
 *
 * N.B. Flag detection (e.g. `std::regex::icase`, `std::regex::multiline`) is a
 * construction-site analysis that is out of scope here.  The `isDotAll` /
 * `isIgnoreCase` / `getFlags` predicates therefore return conservative
 * placeholders; flag-aware analyses belong in a later flow-config PR.
 */
...
/**
 * A `StringLiteral` that is used as a regular expression — either literally
 * passed to `std::regex` or recognised by a downstream flow-config analysis.
 *
 * For now (no dataflow gate), every C++ `StringLiteral` is a candidate so that
 * the parser and tree view can be exercised in library tests.  A later PR will
 * restrict this via `usedAsRegex` / `regexMatchedAgainst`.
 */
abstract class RegExp extends StringLiteral {
  /**
   * Holds if this `RegExp` has the `s` flag for dot-all matching.
   * Not yet detectable without construction-site flag analysis.
   */
  ...
  /**
   * Gets the text of this regex — the value of the string literal as interpreted
   * by the C++ compiler (i.e. `getValue()`, with C++ escape sequences resolved).
   * For example, `"\\d+"` in C++ source has value `\d+`.
   *
   * Note: location columns for terms inside non-raw string literals that contain
   * backslash escape sequences (e.g. `"\\d+"`) are approximate because the
   * value-offset to source-column mapping is not 1:1 for those literals.
   * For precise locations, prefer raw string literals (`R"(\d+)"`).
   */
  ...
  /** Matches named character properties such as `[[:digit:]]` (POSIX bracket expressions valid in ECMAScript mode) */
   ...
   * These are valid in ECMAScript mode (`std::regex` default) inside `[...]`.
   * Examples: `[[:alpha:]]`, `[[:digit:]]`, `[[.a.]]`, `[[=a=]]`.
   */
  predicate posixStyleNamedCharacterProperty(int start, int end, string name) {
   ...
   * Escaped characters include hex values, unicode values and named escapes,
   * but excludes backreferences.
   * Only ECMAScript-relevant escapes are recognised.
   */
  predicate escapedCharacter(int start, int end) {
    ...
    (
      ...
      (
        ...
        not this.getChar(start + 2) = "{" and end = start + 6
        or
        // \u{H...} (variable number of hex digits)
   ...
   * Only ECMAScript special characters are recognised (`^`, `$`, `.`, `\b`, `\B`).
   */
  ...
  /**
   * Matches the start of a named group.
   * ECMAScript supports `(?<name>...)` syntax.
   */
  ...
  private predicate firstPart(int start, int end) {
    ...
    exists(int x | this.firstPart(x, end) |
      ...
      or
      // ^ matches the start of the string (ECMAScript anchor)
  ...
  private predicate lastPart(int start, int end) {
    ...
    exists(int y | this.lastPart(start, y) |
      ...
      or
      // $ matches the end of the string (ECMAScript anchor)
