/**
 * Provides a class hierarchy corresponding to a parse tree of regular
 * expressions, targeting the ECMAScript dialect that is the default for
 * `std::regex`.
 *
 * The module `Impl` satisfies the shared `RegexTreeViewSig` signature so it
 * can be plugged into the shared NFA / ReDoS engine in later PRs.
 *
 * Flag detection (e.g. `std::regex::icase`) is a construction-site analysis
 * that is out of scope here.  `isIgnoreCase`, `isDotAll`, and `getFlags`
 * therefore return conservative placeholders.
 *
 * Location note: term start/end columns are computed by mapping the value
 * offset into the string literal, offset by the length of the opening
 * delimiter (`"` for plain strings; `L"`, `u"`, `U"`, `u8"` for
 * encoding-prefixed strings; `R"delim(` for raw strings).  For plain
 * non-raw string literals that contain C++ escape sequences (e.g. `"\\d+"`),
 * columns are an approximation because the C++ extractor stores the
 * interpreted value (where `\\` → `\`), making the value-offset to
 * source-column mapping non-1:1.  Use raw string literals (`R"(\d+)"`) for
 * precise locations.  Multi-line raw strings are also approximate.
 */
...
private newtype TRegExpParent =
  ...
  /** A named character property (POSIX bracket expression) */
...
private module Impl implements RegexTreeViewSig {
  ...
  class RegExpTerm extends RegExpParent {
     ...
     *
     * The column is computed by adding the value offset to the content start
     * column of the enclosing string literal.  The content start column
     * accounts for the opening delimiter:
     * - plain `"..."` → offset 1
     * - `L"..."`, `u"..."`, `U"..."` → offset 2
     * - `u8"..."` → offset 3
     * - `R"(..."` → offset 3 (for empty custom delimiter)
     * - `R"delim(..."` → offset 3 + len(delim)
     * - encoding + raw (e.g. `LR"(..."`) → sum of above
     *
     * Note: for single-line literals the computation is exact for raw strings
     * and for plain strings without backslash escape sequences.  Plain strings
     * containing backslash sequences (e.g. `"\\d+"`) have approximate columns
     * because the C++ compiler resolves `\\` to `\` before storing the value.
     */
    predicate hasLocationInfo(
      string filepath, int startline, int startcolumn, int endline, int endcolumn
    ) {
      exists(int literalStartCol |
        re.getLocation().hasLocationInfo(filepath, startline, literalStartCol, endline, _) and
        startcolumn = literalStartCol + contentOffset(re) + start and
        endcolumn = literalStartCol + contentOffset(re) + end - 1
      )
    }
  ...
  /**
   * Computes the number of characters from the start of the string literal's
   * source spelling to the first character of the string's value (content).
   *
   * This is the offset added to a value-offset to get the source column relative
   * to the literal's start column.
   *
   * For a plain string `"..."` the result is 1 (skip the opening `"`).
   * For a raw string `R"delim(..."` the result is 3 + len(delim).
   * For encoding-prefixed strings the corresponding prefix length is added.
   */
  private int contentOffset(StringLiteral lit) {
    exists(string vt | vt = lit.getValueText() |
      // Non-raw string: find the opening '"' and add 1
      // The raw value text contains the full source representation including prefix and quotes.
      not exists(int i | vt.substring(i, i + 2) = "R\"") and
      result = min(int i | vt.charAt(i) = "\"") + 1
      or
      // Raw string R"delim(...)delim": find 'R"', then find '(' after it
      exists(int rquotePos, int parenPos |
        rquotePos = min(int i | vt.substring(i, i + 2) = "R\"") and
        parenPos =
          min(int j |
            j > rquotePos + 1 and
            vt.charAt(j) = "("
  ...
  class RegExpCharacterClassEscape extends RegExpEscape {
    // ECMAScript character class escapes: \d \D \s \S \w \W
   ...
   * In ECMAScript mode (`std::regex` default), this covers POSIX bracket
   * expressions such as `[[:digit:]]`.
   */
  additional class RegExpNamedCharacterProperty extends RegExpTerm, TRegExpNamedCharacterProperty {
    ...
    /**
     * Gets the property name. For example, in `[[:space:]]`, the result is
     * `"space"`.
     */
  ...
  predicate isEscapeClass(RegExpTerm term, string clazz) {
    ...
    or
    // POSIX bracket expressions mapped to ECMAScript escape classes
    exists(RegExpNamedCharacterProperty prop | term = prop |
      prop.getName().toLowerCase() = "digit" and
      if prop.isInverted() then clazz = "D" else clazz = "d"
      or
      prop.getName().toLowerCase() = "space" and
      if prop.isInverted() then clazz = "S" else clazz = "s"
      or
      prop.getName().toLowerCase() = "word" and
      if prop.isInverted() then clazz = "W" else clazz = "w"
    )
  }
