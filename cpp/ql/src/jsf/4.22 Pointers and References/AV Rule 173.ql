...
where a.fromSource() and
      ...
      (not global instanceof LocalVariable
       or global.getParentScope() = local.getParentScope().getParentScope+()
       or global.hasSpecifier("static"))
select a, "AV Rule 173: The address of an object with automatic storage shall not be assigned to another object that may persist after the first object has ceased to exist"