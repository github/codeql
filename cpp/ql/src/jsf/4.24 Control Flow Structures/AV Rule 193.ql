...
predicate lastDefaultCase(SwitchCase sc) {
  sc.isDefault() and not exists(SwitchCase sc2 | sc2.getSwitchStmt() = sc.getSwitchStmt() and sc2.getChildNum() > sc.getChildNum())
}

from SwitchCase sc
where sc.fromSource() and
      exists(sc.getAStmt()) and
      not lastDefaultCase(sc) and
      not sc.terminatesInBreakStmt() and
      not sc.terminatesInReturnStmt()
select sc, "AV Rule 193: Every non-empty case clause in a switch statement shall be terminated with a break statement."