...
predicate isValidLoopUpdate(Expr e) {
  ...
  or e instanceof AssignExpr
  or e instanceof FunctionCall  // this is to account for overloaded operators
}

from ForStmt for
where for.fromSource() and
      not isValidLoopUpdate(for.getUpdate())
select for, "AV Rule 199: The increment expression in a for loop will only update a single for loop parameter."