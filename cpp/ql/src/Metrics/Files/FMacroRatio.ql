...
predicate macroLineCount(File f, int num) {
  num = count(int line | macroLines(f, line))
}

from MetricFile f, int macroLines, int loc
where f.fromSource() and loc = f.getNumberOfLinesOfCode() and loc > 0 and
      macroLineCount(f, macroLines)
select f, 100.0 * (macroLines.(float) / loc.(float)) as ratio
order by ratio desc
