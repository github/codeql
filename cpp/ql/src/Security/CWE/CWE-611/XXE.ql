...
Expr asConvertedOrIndirect(DataFlow::Node node) {
  result = [node.asConvertedExpr(), node.asIndirectConvertedExpr()]
}
...
class XXEConfiguration extends DataFlow::Configuration {
  ...
  override predicate isAdditionalFlowStep(
    ...
    state2 = asConvertedOrIndirect(node2).(XxeFlowStateTransformer).transform(state1) and
  ...
  override predicate isBarrier(DataFlow::Node node, string flowstate) {
    ...
    // flowstate value.
    asConvertedOrIndirect(node).(XxeFlowStateTransformer).transform(flowstate) != flowstate
  }
}
