...
pragma[nomagic]
private ReturnPosition viableReturnPosNodeCandFwd1(
  DataFlowCall call, ReturnKindExt kind, Configuration config
) {
  nodeCandFwd1ReturnPosition(result, _, config) and
  result = viableReturnPos(call, kind)
}

pragma[nomagic]
private predicate flowOutOfCallNodeCandFwd1(
  DataFlowCall call, ReturnPosition pos, Node out, Configuration config
) {
  exists(ReturnKindExt kind |
    pos = viableReturnPosNodeCandFwd1(call, kind, config) and
    out = kind.getAnOutNode(call) and
    nodeCandFwd1(out, _, config)
  )
}
...
private predicate nodeCand1ReturnPosition(ReturnPosition pos, Configuration config) {
  exists(DataFlowCall call, Node out |
    ...
    flowOutOfCallNodeCandFwd1(call, pos, out, config)
  )
}
...
pragma[noinline]
private ReturnPosition getReturnPositionNodeCand1(ReturnNodeExt node, Configuration config) {
...
private predicate flowOutOfCallNodeCand1(
  ...
  nodeCand1(out, _, config) and
  flowOutOfCallNodeCandFwd1(call, getReturnPositionNodeCand1(ret, config), out, config) and
...
private predicate nodeCand2(
  ...
  (
    ...
    toReturn = true and
    if nodeCandFwd2(node, true, TBooleanSome(_), _, config)
...
private predicate nodeCand2Out(
  ...
  exists(Node out, boolean allowsFieldFlow |
    flowOutOfCallNodeCand1(call, ret, out, allowsFieldFlow, config) and
    nodeCand2(out, toReturn, returnRead, read, config)
  |
...
private predicate flowCandFwd0(
  ...
  fromArg = true and
  if nodeCand2(node, true, _, _, config)
...
private predicate flowCandFwdIsEntered(
  ...
  exists(ParameterNode p |
    ...
    nodeCand2(p, true, _, unbindBool(apf.toBoolNonEmpty()), config)
  )
}
...
private predicate flowCand0(
  ...
  toReturn = true and
  if flowCandFwd(node, true, _, _, config)
...
private predicate flowCandOut(
  ...
  exists(Node out, boolean allowsFieldFlow |
    flowOutOfCallNodeCand2(call, ret, out, allowsFieldFlow, config) and
    flowCand(out, toReturn, returnApf, apf, config)
  |
    apf instanceof AccessPathFrontNil or allowsFieldFlow = true
  )
}
...
private predicate flowFwd0(
  ...
  fromArg = true and
  if flowCand(node, true, _, _, config)
...
private predicate flow0(
  ...
  toReturn = true and
  if flowFwd(node, true, TAccessPathSome(_), _, _, config)
...
private predicate flowOut(
  ...
  exists(Node out, boolean allowsFieldFlow |
    flowOutOfCallNodeCand2(call, ret, out, allowsFieldFlow, config) and
    flow(out, toReturn, returnAp, ap, config)
  |
    ap instanceof AccessPathNil or allowsFieldFlow = true
  )
}
