...
predicate resolve(Class dynamic, VirtualFunction resolved, VirtualFunction actual) {
  ...
  then (actual = resolved.getAnOverridingFunction*() and
...
predicate lvalue(Element e) {
  ...
  or
  exists(FieldAccess fa | fa.getQualifier().getFullyConverted() = e
    and not(pointerValue(e)))
  or
  exists(Call c | c.getQualifier().getFullyConverted() = e
    and not(pointerValue(e)))
  or
  ...
  exists(PointerDereferenceExpr deref | e = deref and
    deref.getUnderlyingType() instanceof FunctionPointerType)
  or
  exists(Variable v | e = v.getInitializer() and
 ...
 */
private
VariableAccess param_rvalue_access_line(Parameter p, int line) {
 ...
 */
private
VariableAccess pick_rvalue_access(Parameter p) {
 ...
 */
private
predicate has_rvalue_access(Parameter p) {
...
predicate pointerValue(Expr e) {
  exists(Type t | t = e.getType().getUnderlyingType() and
...
{
  exists(Variable v | not(lvalue(dest)) and
    src = v and (dest = v.getAnAccess() or dest = v.getInitializer()))
  or
  exists(AssignExpr assign | not lvalue(assign) and
    src = assign.getLValue().getFullyConverted() and dest = assign)
  or
  ...
  exists(FunctionCall c, Function f, Parameter p, int i |
    p = f.getParameter(i) and not has_rvalue_access(p) and
    dest = c.getArgument(i).getFullyConverted() and not(f.isVirtual()) and
    src = p.getAnAccess() and c.getTarget() = f)
  or
  exists(PointerDereferenceExpr deref | not(lvalue(deref)) and
    src = deref.getOperand().getFullyConverted() and dest = deref)
  or
  exists(ArrayExpr ae | not(lvalue(dest)) and dest = ae and
    src = ae.getArrayBase().getFullyConverted() and pointerValue(src))
  or
  exists(ArrayExpr ae | not(lvalue(dest)) and dest = ae and
  ...
  exists(AggregateLiteral agg | not(lvalue(dest)) and
    ...
    src = agg and dest = agg.getAChild().getFullyConverted())
  or
...
{
  exists(Variable v | lvalue(dest) and
    src = v and (dest = v.getAnAccess() or dest = v.getInitializer()))
  or
  exists(FunctionAccess fa | src = fa.getTarget() and dest = fa)
  or
  exists(AssignExpr assign | lvalue(assign) and
    src = assign.getLValue().getFullyConverted() and dest = assign)
  or
  exists(AddressOfExpr addof | dest = addof and
    src = addof.getOperand().getFullyConverted())
  or
  exists(FunctionCall c, Function f, int i | not lvalue(dest) and
    src = c.getArgument(i).getFullyConverted() and not(f.isVirtual()) and
    dest = pick_rvalue_access(f.getParameter(i)) and c.getTarget() = f)
  or
  exists(FunctionCall c, Function f, int i |
    src = c.getArgument(i).getFullyConverted() and not(f.isVirtual()) and
    c.getTarget() = f and i >= f.getNumberOfParameters() and
    varArgRead(f, dest))
  or
  exists(FunctionCall c, Function f, ReturnStmt r |
    ...
    src = r.getExpr().getFullyConverted() and dest = c)
  or
  exists(PointerDereferenceExpr deref | lvalue(deref) and
    src = deref.getAChild().getFullyConverted() and dest = deref)
  or
  exists(Variable v | dest = v.getInitializer() and
    src = v.getInitializer().getExpr().getFullyConverted())
  or
  exists(ArrayExpr ae | lvalue(dest) and dest = ae and
    src = ae.getArrayBase().getFullyConverted() and pointerValue(src))
  or
  exists(ArrayExpr ae | lvalue(dest) and dest = ae and
    src = ae.getArrayOffset().getFullyConverted() and pointerValue(src))
  or
  exists(Expr arg, BinaryArithmeticOperation binop | dest = binop and src = arg and
    pointerValue(binop) and pointerValue(arg) and
  ...
  or
  exists(IncrementOperation inc | dest = inc and
    src = inc.getOperand().getFullyConverted())
  or
  exists(IncrementOperation dec | dest = dec and
  ...
  or
  exists(AggregateLiteral agg | lvalue(dest) and
    agg.getType().getUnderlyingType() instanceof ArrayType and
    src = agg and dest = agg.getAChild().getFullyConverted())
  or
...
predicate varArgRead(Function f, Expr e)
{
  exists(Macro m, MacroInvocation mi |
    ...
    not(e.getParent() = mi.getAGeneratedElement())
    and e.getEnclosingFunction() = f)
}
...
predicate flowToCompound(Element destParent, string destLabel, Element src)
{
  exists(ExprCall call, int i |
    ...
    if i < call.getType().(FunctionPointerType).getNumberOfParameters() then
      destLabel = "+arg" + i.toString()
    else
      destLabel = "+vararg")
  or
  exists(Function f, ReturnStmt ret |
    ...
    destLabel = "+ret" and destParent = f)
  or
  exists(AggregateLiteral agg, Struct s, int i |
    destParent = agg and lvalue(src) and
...
predicate flowFromCompound(Element parent, string label, Element dest)
{
  ...
  or
  exists(Function f, int i | dest = f.getParameter(i).getAnAccess()
    and label = "+arg" + i.toString() and parent = f)
  or
  exists(Function f |
    parent = f and label = "+vararg" and varArgRead(f, dest)) 
  or
  exists(FieldAccess fa | dest = fa and parent = fa.getQualifier().getFullyConverted()
    and label = fa.getTarget().getName() and lvalue(dest))
  or
  exists(ThisExpr thisexpr | dest = thisexpr and label = "+this"
    and parent = thisexpr.getEnclosingFunction())
  //
  // add more cases here
  //
}
...
predicate aggregateLiteralChild(AggregateLiteral agg, Struct s, int i, Expr child) {
  ...
  exists(Type t |
    t = agg.getType() and agg instanceof AggregateLiteral and s = t.getUnderlyingType() and s instanceof Struct
  )
  and 
...
{
  exists(FieldAccess fa | dest = fa and parent = fa.getQualifier().getFullyConverted()
    and label = fa.getTarget().getName() and not(lvalue(dest)))
  or
  exists(AggregateLiteral agg, Struct s, int i |
    parent = agg and not(lvalue(dest)) and
...
predicate virtualArg(Expr receiver, VirtualFunction called, string arglabel, Expr arg)
{
  exists(FunctionCall c, int i |
    receiver = c.getQualifier().getFullyConverted() and called = c.getTarget() and called.isVirtual()
    and arg = c.getArgument(i) and i >= 0 and
    if i < called.getNumberOfParameters() then
      arglabel = "+arg" + i.toString()
    else
      arglabel = "+vararg")
}
...
predicate virtualThis(Expr receiver, VirtualFunction called, string thislabel, Expr thisexpr)
{
  exists(FunctionCall c |
    receiver = c.getQualifier().getFullyConverted() and called = c.getTarget() and thislabel = "+this"
    and called.isVirtual() and thisexpr = receiver)
}

predicate virtualRet(Expr receiver, VirtualFunction called, string retlabel, FunctionCall c)
{
  receiver = c.getQualifier().getFullyConverted() and called = c.getTarget()
 ...
 */
cached
predicate pointstosets(int ptset, @element elem)
{
  pointstoinfo(_, elem, "", ptset)
}
...
{
  pointstoinfo(parentset, _, label, childset)
  and label != "" and label != "--element--" and label != "--flow--"
}
