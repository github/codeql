...
class PropertyProvider extends IRPropertyProvider {
  override string getInstructionProperty(Instruction instruction, string key) {
    (
      ...
      result = strictconcat(MemoryLocation loc | loc = getResultMemoryLocation(instruction) | loc.toString(), ",")
    ) or
    (
      key = "ResultVirtualVariable" and
      result = strictconcat(MemoryLocation loc | loc = getResultMemoryLocation(instruction) | loc.getVirtualVariable().toString(), ",")
    ) or
    (
      key = "OperandMemoryLocation" and
      result = strictconcat(MemoryLocation loc | loc = getOperandMemoryLocation(instruction.getAnOperand()) | loc.toString(), ",")
    ) or
    (
      key = "OperandVirtualVariable" and
      result = strictconcat(MemoryLocation loc | loc = getOperandMemoryLocation(instruction.getAnOperand()) | loc.getVirtualVariable().toString(), ",")
    ) or
    ...
    exists(MemoryLocation useLocation, IRBlock defBlock, int defRank, int defOffset |
      hasDefinitionAtRank(useLocation, _, defBlock, defRank, defOffset) and
      defBlock.getInstruction(getIndexForOffset(defOffset)) = instruction and
      key = "DefinitionReachesUse" + getKeySuffixForOffset(defOffset) + "[" + useLocation.toString() + "]" and
