...
predicate isES20xxFeature(ASTNode nd, int version, string category) {
  version = 2015 and (
   exists(nd.(Parameter).getDefault()) and category = "default function parameters" or
   exists (Expr e | e = nd and
   ...
   exists (Property prop | prop = nd |
     ...
     prop.isMethod() or
     prop.isComputed()
   ) and category = "object literal extensions" or
   nd instanceof ForOfStmt and category = "for..of loops" or
   nd.(Literal).getRawValue().regexpMatch("^0[bo].*") and category = "octal and binary literals" or
   nd instanceof TemplateLiteral and category = "template literals" or
   ...
   or
   (Parameter)nd instanceof DestructuringPattern and category = "destructuring parameters" or
   exists (StringLiteral sl | sl = nd |
     sl.getRawValue().regexpMatch(".*(?<!\\\\)\\\\u\\{[a-fA-F0-9]+\\}.*") and
     category = "Unicode code point escapes"
   )
   or
   nd instanceof NewTargetExpr and category = "new.target" or
   nd instanceof ConstDeclStmt and category = "const" or
   nd instanceof LetStmt and category = "let" or
   nd instanceof ArrowFunctionExpr and category = "arrow functions" or
   nd instanceof ClassDefinition and category = "class" or
   nd.(Function).isGenerator() and category = "generators" or
   nd instanceof YieldExpr and category = "generators" or
   nd instanceof ES2015Module and category = "modules"
  )
  or
  version = 2016 and (
   (nd instanceof ExpExpr or nd instanceof AssignExpExpr) and category = "exponentiation operator"
  )
  or
  version = 2017 and (
