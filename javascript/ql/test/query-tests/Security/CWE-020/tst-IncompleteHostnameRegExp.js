(function() {
	...
	/^https:\/\/test.example.com/; // NOT OK
	/^https:\/\/test\\.example.com/; // OK
	/^https:\/\/test.example.net/; // NOT OK
	/^https:\/\/test.(example-a|example-b).com/; // NOT OK
	/^https:\/\/(.+).example.com\//; // NOT OK
	/^https:\/\/(\\.+)\\.example.com/; // OK
	/^https:\/\/(?:.+)\\.test\\.example.com\//; // NOT OK
	/^https:\/\/test.example.com\/(?:.*)/; // OK
	new RegExp("^https://test.example.com"); // NOT OK
	s.match("^https://test.example.com"); // NOT OK

	function id(e) { return e; }
	new RegExp(id(id(id("^https://test.example.com")))); // NOT OK
	...
	/^(https:\/\/sub.example.com\/)/g; // NOT OK
	/^https?:\/\/api.example.com/; // NOT OK
	new RegExp('^https://localhost:8000|' + '^https?://.+\\.example\\.com/'); // NOT OK
	...
	new RegExp('^https://test\.example.com'); // NOT OK, but flagged by js/useless-regexp-character-escape

	/^https:\/\/(..|...)\.example\.com\/index\.html/; // OK, wildcards are intentional
	/^https:\/\/.\.example\.com\/index\.html/; // OK, the wildcard is intentional
