/<[^>]*\n?.*=("|')?(.*\.jpg)("|')?.*\n?[^<]*>/g;
/<[^>]*>/g;
/(<\/?)(?i:(?<element>a(bbr|cronym|ddress|pplet|rea)?|b(ase(font)?|do|ig|lockquote|ody|r|utton)?|c(aption|enter|ite|(o(de|l(group)?)))|d(d|el|fn|i(r|v)|l|t)|em|f(ieldset|o(nt|rm)|rame(set)?)|h([1-6]|ead|r|tml)|i(frame|mg|n(put|s)|sindex)?|kbd|l(abel|egend|i(nk)?)|m(ap|e(nu|ta))|no(frames|script)|o(bject|l|pt(group|ion))|p(aram|re)?|q|s(amp|cript|elect|mall|pan|t(r(ike|ong)|yle)|u(b|p))|t(able|body|d|extarea|foot|h|itle|r|t)|u(l)?|var))(\s(?<attr>.+?))*>/g; // $ TODO-SPURIOUS: Alert[js/redos]
/\xA9/g;
...
/"([^"](?:\\.|[^\\"]*)*)"/g; // $ TODO-SPURIOUS: Alert[js/redos]
...
/<(\/{0,1})img(.*?)(\/{0,1})\>/g;
/src[^>]*[^/].(?:jpg|bmp|gif)(?:\"|\')/g;
/<(\w+)(\s(\w*=".*?")?)*((\/>)|((\/*?)>.*?<\/\1>))/g; // $ TODO-SPURIOUS: Alert[js/redos]
...
/^#?(([fFcC0369])\2){3}$/g;
/&(?![a-zA-Z]{2,6};|#[0-9]{3};)/g;
/&lt;\/?([a-zA-Z][-A-Za-z\d\.]{0,71})(\s+(\S+)(\s*=\s*([-\w\.]{1,1024}|&quot;[^&quot;]{0,1024}&quot;|'[^']{0,1024}'))?)*\s*&gt;/g; // $ TODO-SPURIOUS: Alert[js/redos]
/<[a-zA-Z][^>]*\son\w+=(\w+|'[^']*'|"[^"]*")[^>]*>/g;
/>(?:(?<t>[^<]*))/g;
/<[^>]*name[\s]*=[\s]*"?[^\w_]*"?[^>]*>/g;
...
/[0][x][0-9a-fA-F]+/g;
/(\[(\w+)\s*(([\w]*)=('|")?([a-zA-Z0-9|:|\/|=|-|.|\?|&]*)(\5)?)*\])([a-zA-Z0-9|:|\/|=|-|.|\?|&|\s]+)(\[\/\2\])/g; // $ TODO-SPURIOUS: Alert[js/redos]
/%[\-\+0\s\#]{0,1}(\d+){0,1}(\.\d+){0,1}[hlI]{0,1}[cCdiouxXeEfgGnpsS]{1}/g;
/^(Function|Sub)(\s+[\w]+)\([^\(\)]*\)/g;
/^(?<path>(\/?(?<step>\w+))+)(?<predicate>\[(?<comparison>\s*(?<lhs>@\w+)\s*(?<operator><=|>=|<>|=|<|>)\s*(?<rhs>('[^']*'|"[^"]*"))\s*(and|or)?)+\])*$/g; // $ TODO-SPURIOUS: Alert[js/redos]
/^(?<Code>([^"']|"[^"]*")*)'(?<Comment>.*)$/g;
/>(?:(?<t>[^<]*))/g;
/<[a-zA-Z][^>]*\son\w+=(\w+|'[^']*'|"[^"]*")[^>]*>/g;
/<[^>]*name[\s]*=[\s]*"?[^\w_]*"?[^>]*>/g;
