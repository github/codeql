...
/^([a-zA-Z0-9@*#]{8,15})$/g;
/^(?=.*\d).{4,8}$/g;
/^[a-zA-Z]+(([\'\,\.\- ][a-zA-Z ])?[a-zA-Z]*)*$/g; // $ TODO-SPURIOUS: Alert[js/redos]
/((\(\d{3}\) ?)|(\d{3}-))?\d{3}-\d{4}/g;
...
/^\d{5}(-\d{4})?$/g;
/^([a-z0-9]{32})$/g;
/^(([a-zA-Z]:|\\)\\)?(((\.)|(\.\.)|([^\\/:\*\?"\|<>\. ](([^\\/:\*\?"\|<>\. ])|([^\\/:\*\?"\|<>]*[^\\/:\*\?"\|<>\. ]))?))\\)*[^\\/:\*\?"\|<>\. ](([^\\/:\*\?"\|<>\. ])|([^\\/:\*\?"\|<>]*[^\\/:\*\?"\|<>\. ]))?$/g; // $ TODO-SPURIOUS: Alert[js/redos]
/\u00A3/g;
/\u2122/g;
...
/\d{4}-?\d{4}-?\d{4}-?\d{4}/g;
/^(\/w|\/W|[^<>+?$%{}&])+$/g; // $ TODO-SPURIOUS: Alert[js/redos]
...
/^(([a-z])+.)+[A-Z]([a-z])+$/g; // $ TODO-SPURIOUS: Alert[js/redos]
/(?-i:\b\p{Lu}+\b)/g;
...
/\p{N}/g;
/^[a-zA-Z0-9]{1,20}$/g;
/(?s)(?:\e\[(?:(\d+);?)*([A-Za-z])(.*?))(?=\e\[|\z)/g; // $ TODO-SPURIOUS: Alert[js/redos]
/^[^#]([^ ]+ ){6}[^ ]+$/g;
...
/\w?<\s?\/?[^\s>]+(\s+[^"'=]+(=("[^"]*")|('[^\']*')|([^\s"'>]*))?)*\s*\/?>/g; // $ TODO-SPURIOUS: Alert[js/redos]
