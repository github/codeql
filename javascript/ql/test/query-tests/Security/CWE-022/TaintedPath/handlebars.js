...
function init() {
    hb.registerHelper("catFile", function catFile(filePath) {
        return fs.readFileSync(filePath); // $ TODO-SPURIOUS: Alert - SINK (reads file)
    });
    hb.registerHelper("prependToLines", function prependToLines(prefix, filePath) {
        return fs
          .readFileSync(filePath) // $ TODO-SPURIOUS: Alert
          .split("\n")
...
app.get('/some/path1', function (req, res) {
    res.send(data.compiledFileAccess({ path: req.params.path })); // $ TODO-MISSING: Alert - template uses vulnerable catFile
});
...
app.get('/some/path4', function (req, res) {
    res.send(data.compiledMixed({
        prefix: ">>> ",
        path: req.params.path // $ TODO-MISSING: Alert - template uses vulnerable helper
    }));
});
