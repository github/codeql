...
var bad = crypto.randomBytes(1)[0] + crypto.randomBytes(1)[0]; // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]
var bad = crypto.randomBytes(1)[0] * crypto.randomBytes(1)[0]; // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]

const buffer = crypto.randomBytes(bytes);
const digits = [];
for (let i = 0; i < buffer.length; ++i) {
    digits.push(Math.floor(buffer[i] / 25.6)); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]
    digits.push(buffer[i] % 8); // OK - input is a random byte, so the output is a uniformly random number between 0 and 7.
    digits.push(buffer[i] % 100); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]
}

var bad = Number('0.' + crypto.randomBytes(3).readUIntBE(0, 3)); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]
...
function setSteps() {
    ...
    for (const byte of buffer.values()) {
        digits.push(Math.floor(byte / 25.6)); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]
        digits.push(byte % 8); // OK - 8 is a power of 2, so the result is unbiased.
        digits.push(byte % 100); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]
    }
}

const secureRandom = require("secure-random");

var bad = secureRandom(10)[0] + secureRandom(10)[0]; // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]

var goodRandom1 = 5 + secureRandom(10)[0];
var goodRandom2 = 5 + secureRandom(10)[0];
var bad = goodRandom1 + goodRandom2; // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]

var dontFlag = bad + bad; // $ TODO-SPURIOUS: Alert[js/biased-cryptographic-random] - OK - the operands have already been flagged - but flagged anyway due to us not detecting that [INCONSISTENCY].

var good = secureRandom(10)[0] / 0xff; // OK - result is not rounded.
var good = Math.ceil(0.5 - (secureRandom(10)[0] / 25.6)); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm] - division generally introduces bias - but not flagged due to not looking through nested arithmetic.
...
var bad = Math.floor(max * (crypto.randomBytes(1)[0] / 100)); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm] - division by 100 gives bias - but not flagged due to not looking through nested arithmetic.
...
const digits = [];
for (let i = 0; i < 10; i++) {
    digits.push(crypto.randomBytes(1)[0] % 10); // $ Alert TODO-MISSING: Alert[js/weak-cryptographic-algorithm]
}
