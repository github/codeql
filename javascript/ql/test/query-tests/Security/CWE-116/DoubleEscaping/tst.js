...
function badDecode(s) {
  return s.replace(/&amp;/g, "&") // $ TODO-SPURIOUS: Alert
          .replace(/&quot;/g, "\"")
          .replace(/&apos;/g, "'");
}
...
function badDecode3(s) {
  s = s.replace(/&amp;/g, "&"); // $ TODO-SPURIOUS: Alert
  s = s.replace(/&quot;/g, "\"");
  return s.replace(/&apos;/g, "'");
}
...
function badPercentEscape(s) {
  s = s.replace(/&/g, '%26');
  s = s.replace(/%/g, '%25'); // $ TODO-SPURIOUS: Alert
  return s;
}
...
function badEncode(s) {
  ...
  return s.replace(indirect1, "&quot;")
          .replace(indirect2, "&apos;")
          .replace(indirect3, "&amp;"); // $ TODO-SPURIOUS: Alert
}
...
function badEncodeWithReplacer(s) {
  ...
  return s.replace(/["']/g, (c) => repl[c]).replace(/&/g, "&amp;"); // $ TODO-SPURIOUS: Alert
}
