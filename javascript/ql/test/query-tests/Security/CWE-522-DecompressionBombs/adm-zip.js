...
function zipBomb(tarFile) {
    ...
    zipEntries.forEach(function (zipEntry) {
        if (zipEntry.entryName === "my_file.txt") {
            console.log(zipEntry.getData().toString("utf8")); // $ TODO-SPURIOUS: Alert
        }
    });
    // outputs the content of file named 10GB
    console.log(admZip.readAsText("10GB")); // $ TODO-SPURIOUS: Alert
    // extracts the specified file to the specified location
    admZip.extractEntryTo("10GB", "/tmp/", false, true); // $ TODO-SPURIOUS: Alert
    // extracts everything
    admZip.extractAllTo("./tmp", true); // $ TODO-SPURIOUS: Alert
}