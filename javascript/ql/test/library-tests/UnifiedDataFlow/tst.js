...
function t1() {
    function target(...rest) {
        ...
        sink(rest.join(',')); // $ MISSING: hasTaintFlow=t1.1 hasTaintFlow=t1.2
    }
...
function t2() {
    function target(x, ...rest) {
        ...
        sink(rest.join(',')); // $ MISSING: hasTaintFlow=t2.2 hasTaintFlow=t2.3
    }
...
function t8() {
    function finalTarget(x, y, z) {
        ...
        sink(z); // $ hasValueFlow=t8.3 SPURIOUS: hasValueFlow=t8.4
    }
...
function t14() {
    function target(x, y, ...rest) {
        sink(x); // $ MISSING: hasValueFlow=t14.1
        sink(y); // $ MISSING: hasValueFlow=t14.1
        sink(rest.pop()); // $ MISSING: hasValueFlow=t14.1
        sink(rest); // $ MISSING: hasTaintFlow=t14.1
    }
...
function t15() {
    function target(safe, x, y, ...rest) {
        sink(safe);
        sink(x); // $ MISSING: hasValueFlow=t15.1
        sink(y); // $ MISSING: hasValueFlow=t15.1
        sink(rest.pop()); // $ MISSING: hasValueFlow=t15.1
        sink(rest); // $ MISSING: hasTaintFlow=t15.1
    }
    const args = new Array(Math.floor(Math.random() * 10));
...
function t16() {
    ...
    sink(array[0]); // $ MISSING: hasValueFlow=t16.1
    sink(array[1]); // $ MISSING: hasValueFlow=t16.1
    sink(array[2]); // $ MISSING: hasValueFlow=t16.1
    sink(array); // $ MISSING: hasTaintFlow=t16.1
}
