...
function shiftKnown() {
    ...
    sink(array.shift()); // $ MISSING: hasValueFlow=shift.1
    sink(array.shift()); // $ MISSING: hasValueFlow=shift.2
}

function shiftUnknown() {
    const array = new Array(Math.floor(Math.random() * 10));
    array.push(source('shift.unkn'));
    sink(array.shift()); // $ MISSING: hasValueFlow=shift.unkn
    sink(array.shift()); // $ MISSING: hasValueFlow=shift.unkn
    sink(array.shift()); // $ MISSING: hasValueFlow=shift.unkn
}
...
function shiftTaint() {
    ...
    sink(array.shift()); // $ MISSING: hasTaintFlow=shift.directly-tainted
    sink(array.shift()); // $ MISSING: hasTaintFlow=shift.directly-tainted
    sink(array.shift()); // $ MISSING: hasTaintFlow=shift.directly-tainted
}
...
function implicitToString() {
    ...
    sink(array.toString()); // $ MISSING: hasTaintFlow=implicitToString.1 hasTaintFlow=implicitToString.2
    sink(array.toString("utf8")); // $ MISSING: hasTaintFlow=implicitToString.1 hasTaintFlow=implicitToString.2
