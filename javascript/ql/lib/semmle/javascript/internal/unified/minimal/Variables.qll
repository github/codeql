...
class Variable extends @variable, LexicalName {
  ...
  /** Like `getAnAccess()` but includes `this` expressions, which are not typed as `VarAccess`. */
  private Expr getAnAccessOrThisExpr() { bind(result, this) }
  ...
  predicate isCaptured() {
    ...
    this.getAnAccessOrThisExpr().getContainer().getFunctionBoundary() !=
...
class LocalVariable extends Variable {
  ...
  /**
   * Alias for `getDeclaringContainer()`.
   */
