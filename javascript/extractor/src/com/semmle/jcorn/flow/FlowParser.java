...
/**
 * Java port of the <a href="https://github.com/babel/babylon">Babylon</a>
 * <a href="https://github.com/babel/babylon/blob/master/src/plugins/flow.js">Flow plugin</a>.
 * All Flow annotations are simply discarded after parsing.
 */
public class FlowParser extends ESNextParser {
	private boolean inType = false, noAnonFunctionType = false;

	private static final TokenType braceBarL = new TokenType(new Properties("{|").beforeExpr().startsExpr());
	...
	/**
	 * Utility class for saving the parser state so we can later either restore
	 * it by calling method {@link #reset()}, or commit to the new state by
	 * calling method {@link #commit()}.
	 */
	private class State {
	...
	private void flowParseDeclare(Position start) {
		if (this.type == TokenType._class) {
			...
			if (".".equals(lookahead(1)))
				...
				this.flowParseDeclareModule(start);
	...
	private void flowParseDeclareExportDeclaration(Position start) {
		...
		if (this.eat(TokenType._default)) {
			...
			if (this.match(TokenType._var) || // declare export var ...
				this.match(TokenType._function) || // declare export function ...
				this.match(TokenType._class) || // declare export class ...
				this.isContextual("opaque") // declare export opaque ..
				) {
				this.flowParseDeclare(this.startLoc);
				return;
			} else if (this.match(TokenType.star) || // declare export * from ''
						this.match(TokenType.braceL) || // declare export {} ...
	...
	private void flowObjectTypeSemicolon() {
		if (!this.eat(TokenType.semi) && !this.eat(TokenType.comma) &&
				this.type != TokenType.braceR && this.type != braceBarR) {
			this.unexpected();
		}
	}
	...
	private void flowParseTupleType() {
		...
		while (this.pos < this.input.length() && this.type != TokenType.bracketR) {
			this.flowParseType();
			if (this.type == TokenType.bracketR)
				break;
			this.expect(TokenType.comma);
		}
		this.expect(TokenType.bracketR);
	...
	private String lookahead(int n) {
		...
		if (m.find(this.pos))
			return inputSubstring(m.end(), m.end()+n);
		return "";
	}
	...
	private void flowParseAnonFunctionWithoutParens() {
		this.flowParsePrefixType();
		if (!noAnonFunctionType && eat(TokenType.arrow))
			flowParseType();
	}
	...
	private void flowParseTypeAnnotatableIdentifier(boolean requireTypeAnnotation, boolean canBeOptionalParam) {
		this.parseIdent(false);
	...
	protected Statement parseStatement(boolean declaration, boolean topLevel, Set<String> exports) {
		...
		if (flow() && declaration && this.strict && this.type == TokenType.name && "interface".equals(this.value)) {
			Position start = startLoc;
			this.next();
	...
	@Override
	protected ExpressionStatement parseExpressionStatement(boolean declaration, Position start, Expression expr) {
		if (flow() && declaration && expr instanceof Identifier) {
			String name = ((Identifier) expr).getName();
			if ("declare".equals(name)) {
				if (this.type == TokenType._class || this.type == TokenType.name ||
						this.type == TokenType._function || this.type == TokenType._var || this.type == TokenType._export) {
	...
	protected boolean shouldParseExportStatement() {
		return flow() && (this.isContextual("type") || this.isContextual("interface") || this.isContextual("opaque"))
	...
	protected Expression parseClassSuper() {
		...
		if (flow() && ext != null && this.isRelational("<"))
			this.flowParseTypeParameterInstantiation();
