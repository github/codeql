...
public class JcornWrapper {
	 ...
	 */
	public static JSParser.Result parse(ExtractorConfig config, SourceType sourceType, String source) {
		...
		Options options = new Options()
				...
				.preserveParens(true).allowReturnOutsideFunction(true);
		if (config.isMozExtensions())
			options.mozExtensions(true);
		if (config.isJscript())
			options.jscript(true);
		if (config.isJsx())
			options = new JSXOptions(options);
		if (config.isEsnext())
			options.esnext(true);
