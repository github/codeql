...
public class ExprKinds {
	...
	private static final Map<IdContext, Integer> idKinds = new EnumMap<IdContext, Integer>(IdContext.class);
	static {
	...
	public static int getExprKind(final Expression expr, final IdContext idContext) {
		Integer kind = expr.accept(new DefaultVisitor<Void, Integer>() {
			@Override
			...
			public Integer visit(Literal nd, Void q) {
				...
				if (tp == TokenType._null)
					return 1;
				if (tp == TokenType._true || tp == TokenType._false)
					return 2;
				if (tp == TokenType.num)
					return 3;
				if (tp == TokenType.string || tp == JSXParser.jsxText)
					return 4;
				if (tp == TokenType.regexp)
					return 5;
				if (tp == TokenType.bigint)
					return 106;
			...
			public Integer visit(UpdateExpression nd, Void q) {
				return nd.isPrefix() ? prefixOpKinds.get(nd.getOperator())
			...
			public Integer visit(MetaProperty nd, Void c) {
				if (nd.getMeta().getName().equals("new"))
					return 82; // @newtargetexpr
