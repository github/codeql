...
/**
 * Extractor for populating lexical information about a JavaScript file, including comments and tokens.
 */
public class LexicalExtractor {
	...
	public LexicalExtractor(TextualExtractor textualExtractor, List<Token> tokens, List<Comment> comments) {
		...
		if (tokens != null && comments != null) {
			...
			for (int i = findNode(tokens, start), m = tokens.size(); i<m; ++i) {
				...
				if (token.getLoc().getStart().compareTo(end) >= 0 || token.getType() == Token.Type.EOF)
					break;
				for (int j = token.getLoc().getStart().getLine(), n = token.getLoc().getEnd().getLine(); j <= n; ++j) {
					if (j > maxline) {
			...
			for (int i = findNode(comments, start), m = comments.size(); i<m; ++i) {
				...
				if (comment.getLoc().getStart().compareTo(end) >= 0)
					break;
				for (int j = comment.getLoc().getStart().getLine(), n = comment.getLoc().getEnd().getLine(); j <= n; ++j) {
					if (j > maxline) {
	...
	private <T extends SourceElement> int findNode(List<T> ts, Position start) {
		...
		/**
		 * Invariant: forall i in [0..lo), ts[i] starts before `start`
		 *        and forall i in [hi..ts.size()), ts[i] starts at or after `start`
		 */
		while (lo < hi) {
			// other parts of the extractor will break long before this could overflow
			mid = (lo + hi) >>> 1;
			if (ts.get(mid).getLoc().getStart().compareTo(start) >= 0)
				hi = mid;
			else
				lo = mid+1;
		}
	...
	public void extractTokens(Label toplevelKey) {
		...
		for (int i=0, n=tokens.size(), idx=0; i<n; ++i) {
			...
			case EOF: kind = 0; break;
			case NULL: kind = 1; break;
			case FALSE: case TRUE: kind = 2; break;
			case NUM: kind = 3; break;
			case STRING: kind = 4; break;
			case REGEXP: kind = 5; break;
			case NAME: kind = 6; break;
			case KEYWORD: kind = 7; break;
			case PUNCTUATOR: kind = 8; break;
			}
			...
			if (i+1 < n) {
				...
				if (token.getLoc().equals(next.getLoc()))
					tokens.set(i+1, null);
			}
		}
	}
	...
	public void extractComments(Label toplevelKey) {
		for (Comment comment : comments) {
			...
			case LINE: kind = 0; break;
			case HTML_START: kind = 3; break;
			case HTML_END: kind = 4; break;
			case BLOCK:
