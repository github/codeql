...
public class NodeJSDetector {
	/**
	 * Is {@code ast} a program that looks like Node.js code, that is,
	 * does it contain a top-level {@code require} or an export?
	 */
	...
	/**
	 * Does this program contain a statement that looks like a Node.js {@code require}
	 * or an export?
	 *
	 * We recursively traverse argument-less immediately invoked function expressions
	 * (i.e., no UMD modules), but not loops or if statements.
	 */
	private static boolean hasToplevelRequireOrExport(List<Statement> stmts) {
		for (Statement stmt : stmts)
			if (hasToplevelRequireOrExport(stmt))
				return true;
		return false;
	}
	...
	private static boolean hasToplevelRequireOrExport(Statement stmt) {
		if (stmt instanceof ExpressionStatement) {
			...
			if (e instanceof UnaryExpression)
				e = ((UnaryExpression) e).getArgument();
	...
	/**
	 * Is {@code e} a call to a function named {@code require} with one argument,
	 * or an assignment whose right hand side is the result of such a call?
	 */
	 ...
	 * </ul>
	 *
	 * Detection is done recursively, so <code>foo = exports.foo = ...</code>
	...
	private static boolean isExport(Expression e) {
		if (e instanceof AssignmentExpression) {
			...
			if (assgn.getLeft() instanceof MemberExpression) {
				...
				// `exports.foo = ...;`
				if (isIdentifier(target.getObject(), "exports"))
					return true;
