...
/**
 * Tests for the JSX parser, automatically generated from the acorn-jsx test suite
 * as described in <code>parser-tests/jcorn-jsx/README.md</code>.
 */
...
public class JSXTests extends ASTMatchingTests {
	...
	public static Iterable<Object[]> tests() {
		...
		for (File test : BASE.listFiles(FileUtil.extensionFilter(true, ".js"))) {
			...
			if (optionsFile.exists()) {
				JsonObject optionsJson = new JsonParser().parse(new WholeIO().strictread(optionsFile)).getAsJsonObject();
				if (optionsJson.has("allowNamespacedObjects"))
					options = options.allowNamespacedObjects(optionsJson.get("allowNamespacedObjects").getAsBoolean());
			...
			if (expectedASTFile.exists())
				expectedAST = new JsonParser().parse(new WholeIO().strictread(expectedASTFile)).getAsJsonObject();
			else
				expectedAST = null;
	...
	public JSXTests(String testName, JSXOptions options, JsonObject expectedAST, String expectedFailure) {
