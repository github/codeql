...
/**
 * Base class for tests that use Acorn-like AST templates to specify
 * expected test output.
 */
public abstract class ASTMatchingTests {
	/**
	 * Assert that the given actual sub-AST matches the expected AST template, where
	 * {@code path} is the path from the root to the sub-AST.
	 */
	protected void assertMatch(String path, JsonElement expected, JsonElement actual) {
		if (expected == null)
			Assert.assertNull(path + ": null != " + actual, actual);
