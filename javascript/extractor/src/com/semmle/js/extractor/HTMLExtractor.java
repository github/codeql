...
public class HTMLExtractor implements IExtractor {
	 ...
	 */
	private final static Pattern JS_ATTRIBUTE = Pattern.compile("^on(abort|blur|change|(dbl)?click|error|focus|key(down|press|up)|load|mouse(down|move|out|over|up)|re(set|size)|select|submit|unload)$", Pattern.CASE_INSENSITIVE);
			...
			if (elt.getName().equals(HTMLElementName.SCRIPT)) {
				...
				if (sourceType != null) {
					...
					if (!source.trim().isEmpty()) {
						...
						snippetLoC = extractSnippet(1, config.withSourceType(sourceType), scopeManager, textualExtractor, source,
								contentStart.getRow(), contentStart.getColumn());
					}
				}
			} else {
					...
					for (Attribute attr : attributes) {
						...
						if (JS_ATTRIBUTE.matcher(attr.getName()).matches()) {
							snippetLoC = extractSnippet(2, config, scopeManager, textualExtractor, source,
									valueStart.getRow(), valueStart.getColumn());
							...
							snippetLoC = extractSnippet(3, config, scopeManager, textualExtractor, source,
									valueStart.getRow(), valueStart.getColumn()+11);
						}
					}
			}
	 ...
	 * element should be extracted, returning <code>null</code> if it cannot
	 * be determined.
	 */
	...
	/**
	 * Get the value of attribute <code>attr</code> of element <code>elt</code>
	 * in lower case; if the attribute has no value, <code>null</code> is returned.
	 */
	...
	private LoCInfo extractSnippet(int toplevelKind, ExtractorConfig config,
			ScopeManager scopeManager, TextualExtractor textualExtractor,
			String source, int line, int column) {
		TrapWriter trapwriter = textualExtractor.getTrapwriter();
		LocationManager locationManager = textualExtractor.getLocationManager();
		LocationManager scriptLocationManager = new LocationManager(locationManager.getSourceFile(), trapwriter, locationManager.getFileLabel());
		scriptLocationManager.setStart(line, column);
		JSExtractor extractor = new JSExtractor(config);
		try {
			TextualExtractor tx = new TextualExtractor(trapwriter, scriptLocationManager, source, config.getExtractLines());
	...
	/**
	 * Is {@code elt} a plain HTML element (as opposed to a doctype declaration, comment,
	...
	/**
	 * Populate the {@code xmlElements} relation recording information about all child
	 * elements of {@code parent}, which is either an {@link Element} or a {@link Source}
	 * (representing the HTML file itself).
	 */
	private void extractChildElements(Segment parent, LocationManager locationManager) {
		...
		int contentStart = parent instanceof Element ? ((Element) parent).getStartTag().getEnd() : parent.getBegin();
		int contentEnd;
		if (parent instanceof Element && ((Element) parent).getEndTag() != null)
			contentEnd = ((Element) parent).getEndTag().getBegin();
		else
			contentEnd = parent.getEnd();
		int prevChildEnd = contentStart;
		for (Element child : parent.getChildElements()) {
			childIndex += emitXmlChars(source, prevChildEnd, child.getBegin(), parentLabel, childIndex, false, fileLabel, locationManager);

			if (isCDataElement(child)) {
				// treat CDATA sections as text
				childIndex += emitXmlChars(source, child.getBegin() + "<![CDATA[".length(), child.getEnd() - "]]>".length(), parentLabel, childIndex, true, fileLabel, locationManager);
			}

			if (isPlainElement(child)) {
				String childName = child.getName();
				Label childLabel = trapWriter.localID(child);
				trapWriter.addTuple("xmlElements", childLabel, childName, parentLabel, childIndex++, fileLabel);
		...
		emitXmlChars(source, prevChildEnd, contentEnd, parentLabel, childIndex, false, fileLabel, locationManager);
	}

	/**
	 * Populate the {@code xmlAttrs} relation recording information about all attributes
	 * of {@code elt}.
	 */
	...
	/**
	 * Record the location of {@code s}, which is either an {@link Element} or
	 * an {@code Attribute}.
	 */
	...
	/**
	 * If {@code textEnd} is greater than {@code textBegin}, extract all characters
	 * in that range as HTML text, populating {@code xmlChars} and make it the 
	 * {@code i}th child of {@code parentLabel}.
	 *
	 * @return 1 if text was extractod, 0 otherwise
	 */
	private int emitXmlChars(Source src, int textBegin, int textEnd, Label parentLabel, int id, boolean isCData, Label fileLabel,
