 ...
 *
 * <p>
 * The extractor uses <a href="http://www.snakeyaml.org/">SnakeYAML</a> to parse YAML.
 * </p>
 */
public class YAMLExtractor implements IExtractor {
    ...
    private static enum YAMLTables implements Table {
        ...
        YAML_SCALARS(3), // yaml_scalars (scalar: @yaml_scalar_node ref, style: int ref, value: string ref)
     ...
     * | 3 = @yaml_alias_node
     */
    private static enum NodeKind { SCALAR, MAPPING, SEQUENCE, ALIAS };
    ...
    /**
     * The underlying SnakeYAML parser; we use the relatively low-level {@linkplain Parser}
     * instead of the more high-level {@linkplain Composer}, since our dbscheme represents
     * YAML documents in AST form, with aliases left unresolved.
     */
    ...
    public LoCInfo extract(TextualExtractor textualExtractor) {
        ...
        try {
            ...
            trapWriter.addTuple(YAMLTables.YAML_ERRORS, trapWriter.freshLabel(), 
            		"Unexpected character " + s + "(" + c + ")");
            // unfortunately, SnakeYAML does not provide structured location information for ReaderExceptions
        }
    ...
    private void extractNode(Label parent, int idx) {
        ...
        if (start.is(Event.ID.Alias)) {
            ...
            if (anchor != null)
                trapWriter.addTuple(YAMLTables.YAML_ANCHORS, label, anchor);

            if (start.is(Event.ID.Scalar)) {
                kind = NodeKind.SCALAR;
                ScalarEvent scalar = (ScalarEvent) start;
                tag = getTag(scalar.getTag(), NodeId.scalar, scalar.getValue(), scalar.getImplicit().canOmitTagInPlainScalar());
                trapWriter.addTuple(YAMLTables.YAML_SCALARS, label, (int)scalar.getStyle(), scalar.getValue());
        ...
        trapWriter.addTuple(YAMLTables.YAML, label, kind.ordinal(), parent, idx, tag, mkToString(start.getStartMark(), end.getEndMark()));
    ...
    /**
     * SnakeYAML doesn't directly expose the source text of nodes, but we can get a decent approximation
     * from the snippet associated with the node's start {@linkplain Mark}.
     *
     * <p>
     * The snippet of a {@linkplain Mark} is meant to be used for diagnostic messages and consists of
     * two lines: the first line contains some context around the source position represented by the mark,
     * the second line contains a caret character positioned underneath the source position itself.
     * </p>
     *
     * <p>
     * To approximate the source text, we take the text on the first line and strip off the first <i>n</i>
     * characters, where <i>n</i> is the number of spaces preceding the caret character on the second line.
     * </p>
     *
     * <p>
     * This is only an approximation, since the context never extends across newlines, but it suffices
     * for the purposes of <code>toString</code>.
     * </p>
     */
