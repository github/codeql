...
/**
 * The syntactic context manager keeps track of the statement currently being
 * extracted and its enclosing loops, switch statements and labelled statements.
 * It can use this information to determine the targets of 'break' or 'continue'
 * statements.
 *
 * The syntactic context manager maintains a context chain. Each context object
 * on the chain represents a syntactic context within a given function or the
 * toplevel, and has a link to its enclosing syntactic context. Context objects
 * also maintain a reference to the statement currently being extracted, and to
 * the function or toplevel.
 *
 * Furthermore, each context object has two maps, one representing 'break' targets,
 * the other 'continue' targets. These maps associate labels <i>l</i>
 * with the nearest enclosing statement (in the same function) that has label
 * <i>l</i>, if any. If <i>l</i> is the empty string, it maps to the nearest enclosing loop
 * or switch statement (but only for 'break' targets).
 */
public class SyntacticContextManager {
	...
	public Object getTarget(JumpStatement nd) {
		...
		if (nd instanceof BreakStatement)
			...
			return this.context.continueTargets == null ? null : this.context.continueTargets.lookup(label);
	}
