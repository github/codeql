...
private int getBlockOrderingInFunction(Function f, BasicBlock bb) {
  bb =
    rank[result + 1](BasicBlock bb2, QlBuiltins::BigInt index0, int index1, int index2,
      int isEntryPoint |
      bb2.getEnclosingFunction() = f and
      bb2.getFirstInstruction().hasInternalOrder(index0, index1, index2) and
      if bb2.isFunctionEntryBasicBlock() then isEntryPoint = 0 else isEntryPoint = 1
    |
      bb2 order by isEntryPoint, index0, index1, index2
    )
}
...
private import internal.InstructionTag
...
private class PrintableInstruction extends PrintableNode, TPrintableInstruction {
  ...
  override string toString() {
    exists(string tail |
      not exists(instr.getAnOperand()) and
      tail = ""
      or
      tail =
        concat(OperandTag tag, string s |
          s = instr.getOperand(tag).toString()
        |
          s order by getOperandTagIndex(tag), " "
        )
    |
      result = instr.toString() + tail
    )
  }
...
private int getSuccessorIndex(BasicBlock pred, BasicBlock succ) {
  succ =
    rank[result + 1](BasicBlock aSucc, QlBuiltins::BigInt index0, int index1, int index2 |
      aSucc = pred.getASuccessor() and
      aSucc.getFirstInstruction().hasInternalOrder(index0, index1, index2)
    |
      aSucc order by index0, index1, index2
    )
}
