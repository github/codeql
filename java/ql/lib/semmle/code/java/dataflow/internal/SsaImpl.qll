...
private predicate untrackedFieldWrite(BasicBlock bb, int i, SsaSourceVariable v) {
  v =
    any(SsaSourceField nf | bb.getNode(i + 1) = nf.getAnAccess().(FieldRead) and not trackField(nf))
}
...
private module SsaInput implements SsaImplCommon::InputSig<Location> {
  ...
  predicate variableRead(BasicBlock bb, int i, SourceVariable v, boolean certain) {
    exists(VarRead use | v.getAnAccess() = use and bb.getNode(i) = use and certain = true)
    or
