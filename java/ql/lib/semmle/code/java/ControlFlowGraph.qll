...
module Ast implements AstSig<Location> {
  ...
  class Switch extends AstNode {
    Switch() {
      ...
      this instanceof SwitchExpr or
      this instanceof WhenExpr
    }

    Expr getExpr() {
      result = this.(SwitchStmt).getExpr() or
      result = this.(SwitchExpr).getExpr()
      // WhenExpr is represented with the expression as part of each branch condition
    }

    Case getCase(int index) {
      result = this.(SwitchStmt).getCase(index) or
      result = this.(SwitchExpr).getCase(index) or
      result = this.(WhenExpr).getBranch(index)
    }
  }
  ...
  class Case extends AstNode {
    Case() { this instanceof J::SwitchCase or this instanceof WhenBranch }

    /** Gets a pattern being matched by this case. */
    AstNode getAPattern() {
      result = this.(PatternCase).getAPattern() or
      result = this.(ConstCase).getValue(_)
      // When branches do not have patterns, only conditions
    }

    /** Gets the guard expression of this case, if any. */
    Expr getGuard() {
      result = this.(PatternCase).getGuard() or result = this.(WhenBranch).getCondition()
    }
    ...
    AstNode getBodyElement(int index) {
      ...
      or
      result = this.(WhenBranch).getRhs() and index = 0
      or
  ...
  predicate fallsThrough(Case c) { not c.(J::SwitchCase).isRule() and not c instanceof WhenBranch }
...
private module Input implements InputSig1, InputSig2 {
  ...
  predicate inConditionalContext(Ast::AstNode n, ConditionKind kind) {
    kind.isBoolean() and
    any(AssertStmt assertstmt).getExpr() = n
  }
