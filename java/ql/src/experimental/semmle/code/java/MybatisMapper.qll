...
/**
 * A `<include>` element in a `mapper.xml` file.
 */
class MybatisInclude extends MapperXMLElement {
  MybatisInclude() { this.getName() = "include" }

  /**
   * Gets the `refid` element of this `<include>`.
   */
  XMLAttribute getRefId() { result = getAttribute("refid") }
}
