 ...
 * definitions are passed in via the `DataFlowImplSpecific` and `FlowSummarySpecific`
 * modules.
 */

private import FlowSummarySpecific
...
module Private {
  ...
  private newtype TSummaryNodeState =
    TSummaryNodeInputState(SummaryComponentStack l) {
      exists(SummaryComponentStack input |
        summary(_, input, _, _) and
        l = input.drop(_)
      )
    } or
    TSummaryNodeOutputState(SummaryComponentStack l) {
   ...
   *
   * - `TSummaryNodeInputState(SummaryComponentStack l)`:
   *   this state represents that the components in `l` have been read from the
   *   input.
   * - `TSummaryNodeOutputState(SummaryComponentStack l)`:
   *   this state represents that the contents of `l` remains to be written to
   *   the output.
   */
  class SummaryNodeState extends TSummaryNodeState {
    /**
     * Holds if this state can represent that the elements of `l` have been read in `c`.
     */
    pragma[nomagic]
    predicate isInputState(SummarizedCallable c, SummaryComponentStack l) {
      this = TSummaryNodeInputState(l) and
      exists(SummaryComponentStack input |
    ...
    /**
     * Holds if this state can represent that the elements of `l` remain to be written in `c`.
     */
    pragma[nomagic]
    predicate isOutputState(SummarizedCallable c, SummaryComponentStack l) {
      this = TSummaryNodeOutputState(l) and
      exists(SummaryComponentStack output |
    ...
    string toString() {
      exists(SummaryComponentStack l |
        this = TSummaryNodeInputState(l) and
        result = "read: " + l
      )
      or
   ...
   */
  private predicate isParameterReadState(SummarizedCallable c, SummaryNodeState state, int i) {
  ...
  predicate summaryNodeRange(SummarizedCallable c, SummaryNodeState state) {
    ...
    not isParameterReadState(c, state, _)
    or
  ...
  predicate summaryParameterNodeRange(SummarizedCallable c, int i) {
    isParameterReadState(c, _, i)
    or
    isParameterPostUpdate(_, c, i)
  }

  pragma[noinline]
  private Node summaryNodeInputState(SummarizedCallable c, SummaryComponentStack l) {
    exists(SummaryNodeState state | state.isInputState(c, l) |
      result = summaryNode(c, state)
      or
      exists(int i |
        isParameterReadState(c, state, i) and
  ...
  pragma[noinline]
  private Node summaryNodeOutputState(SummarizedCallable c, SummaryComponentStack l) {
    exists(SummaryNodeState state |
      state.isOutputState(c, l) and
      result = summaryNode(c, state)
    )
  }
  ...
  private predicate callbackOutput(
    SummarizedCallable c, SummaryComponentStack l, Node receiver, ReturnKind rk
  ) {
    any(SummaryNodeState state).isInputState(c, l) and
    l.head() = TReturnSummaryComponent(rk) and
    receiver = pre(summaryNodeInputState(c, l.drop(1)))
  }

  private predicate callbackInput(
    SummarizedCallable c, SummaryComponentStack l, Node receiver, int i
  ) {
    any(SummaryNodeState state).isOutputState(c, l) and
    l.head() = TParameterSummaryComponent(i) and
    receiver = pre(summaryNodeOutputState(c, l.drop(1)))
  }
  ...
  DataFlowType summaryNodeType(Node n) {
    ...
    or
    exists(SummarizedCallable c, SummaryComponentStack l, SummaryComponent head | head = l.head() |
      n = summaryNodeInputState(c, l) and
      (
        exists(Content cont |
        ...
        exists(ReturnKind rk |
          ...
          result = getCallbackReturnType(getNodeType(summaryNodeInputState(c, l.drop(1))), rk)
        )
      )
      or
      n = summaryNodeOutputState(c, l) and
      (
        exists(Content cont |
          head = TContantSummaryComponent(cont) and result = getContentType(cont)
        )
        ...
        exists(int i | head = TParameterSummaryComponent(i) |
          result = getCallbackParameterType(getNodeType(summaryNodeOutputState(c, l.drop(1))), i)
        )
      )
    )
  }
  ...
  predicate summaryOutNode(DataFlowCall c, Node out, ReturnKind rk) {
    exists(SummarizedCallable callable, SummaryComponentStack l, Node receiver |
      callbackOutput(callable, l, receiver, rk) and
      out = summaryNodeInputState(callable, l) and
  ...
  predicate summaryArgumentNode(DataFlowCall c, Node arg, int i) {
    exists(SummarizedCallable callable, SummaryComponentStack l, Node receiver |
      callbackInput(callable, l, receiver, i) and
      arg = summaryNodeOutputState(callable, l) and
      c = summaryDataFlowCall(receiver)
    )
  }
  ...
  predicate summaryReturnNode(Node ret, ReturnKind rk) {
    exists(SummarizedCallable callable, SummaryComponentStack l |
      ret = summaryNodeOutputState(callable, l) and
      l = TSingletonSummaryComponentStack(TReturnSummaryComponent(rk))
    )
  }
  ...
  module Steps {
    ...
    predicate summaryReadStep(Node pred, Content c, Node succ) {
      exists(SummarizedCallable sc, SummaryComponentStack l |
        pred = summaryNodeInputState(sc, l.drop(1)) and
        succ = summaryNodeInputState(sc, l) and
        SummaryComponent::content(c) = l.head()
      )
    }
    ...
    predicate summaryStoreStep(Node pred, Content c, Node succ) {
      exists(SummarizedCallable sc, SummaryComponentStack l |
        pred = summaryNodeOutputState(sc, l) and
        succ = summaryNodeOutputState(sc, l.drop(1)) and
        SummaryComponent::content(c) = l.head()
      )
    }
  ...
  module TestOutput {
    ...
    query predicate summary(string callable, string flow, boolean preservesValue) {
      ...
      |
        ...
        summary(c, input, output, preservesValue) and
        flow = input + " -> " + output
      )
    }
  }
