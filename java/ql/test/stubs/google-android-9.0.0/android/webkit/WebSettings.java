 ...
 * {@link WebView#getSettings()} is tied to the life of the WebView. If a
 * WebView has been destroyed, any method call on {@code WebSettings} will throw
...
public abstract class WebSettings {
     ...
     * <ul>
     * <li>{@code NORMAL} means no rendering changes. This is the recommended choice
     * for maximum compatibility across different platforms and Android
     * versions.</li>
     * <li>{@code SINGLE_COLUMN} moves all content into one column that is the width
     * of the view.</li>
     * <li>{@code NARROW_COLUMNS} makes all columns no wider than the screen if
     * possible. Only use this for API levels prior to
     * {@link android.os.Build.VERSION_CODES#KITKAT}.</li>
     * <li>{@code TEXT_AUTOSIZING} boosts font size of paragraphs based on
     * heuristics to make the text readable when viewing a wide-viewport layout in
     * the overview mode. It is recommended to enable zoom support
     * {@link #setSupportZoom} when using this mode. Supported from API level
     * {@link android.os.Build.VERSION_CODES#KITKAT}</li>
     * </ul>
     */
     ...
     * </ul>
     *
     * @deprecated Use {@link WebSettings#setTextZoom(int)} and
     *             {@link WebSettings#getTextZoom()} instead.
     */
    @Deprecated
     ...
     * otherwise load resources from the network. Use with {@link #setCacheMode}.
     */
     ...
     *             {@link android.os.Build.VERSION_CODES#HONEYCOMB} and onwards it
     *             has the same effect as {@link #LOAD_DEFAULT}.
     */
    @Deprecated
     ...
     * content, a placeholder is shown until the user clicks on the placeholder.
     * Once clicked, the plugin will be enabled on the page. OFF means that all
     * plugins will be turned off and any fallback content will be used.
     */
     ...
     *
     * In this mode, the WebView will allow a secure origin to load content from any
     * other origin, even if that origin is insecure. This is the least secure mode
     * of operation for the WebView, and where possible apps should not set this
     * mode.
     */
     ...
     * Used with {@link #setMixedContentMode}
     *
     * In this mode, the WebView will not allow a secure origin to load content from
     * an insecure origin. This is the preferred and most secure mode of operation
     * for the WebView and apps are strongly advised to use this mode.
     */
    public static final int MIXED_CONTENT_NEVER_ALLOW = 1;
    /**
     * Used with {@link #setMixedContentMode}
     *
     * In this mode, the WebView will attempt to be compatible with the approach of
     * a modern web browser with regard to mixed content. Some insecure content may
     * be allowed to be loaded by a secure origin and other types of content will be
     * blocked. The types of content are allowed or blocked may change release to
     * release and are not explicitly defined.
     *
     * This mode is intended to be used by apps that are not in control of the
     * content that they render but desire to operate in a reasonably secure
     * environment. For highest security, apps are recommended to use
     ...
     * be set with {@link #setBuiltInZoomControls}. This setting does not affect
     * zooming performed using the {@link WebView#zoomIn()} and
    ...
    /**
     * Sets whether the WebView requires a user gesture to play media. The default
     * is {@code true}.
     *
     ...
     * control zooming. Whether or not these on-screen controls are displayed can be
     * set with {@link #setDisplayZoomControls}. The default is {@code false}.
     * <p>
     * The built-in mechanisms are the only currently supported zoom mechanisms, so
     ...
     * the built-in zoom mechanisms. See {@link #setBuiltInZoomControls}. The
     ...
     *
     * @return {@code true} if the WebView displays on-screen zoom controls when
     ...
     * content to fit on screen by width. This setting is taken into account when
     * the content width is greater than the width of the WebView control, for
     * example, when {@link #getUseWideViewPort} is enabled. The default is
     * {@code false}.
     */
     ...
     * zooming or while the window hosting the WebView does not have focus. If it is
     * {@code true}, WebView will choose a solution to maximize the performance.
     * e.g. the WebView's content may not be updated during the transition. If it is
     * false, WebView will keep its fidelity. The default value is {@code false}.
     *
     ...
     * {@code true}, it will use the WebView's background. If {@code false}, it will
     * use an internal pattern. Default is {@code true}.
     *
     * @deprecated This method is now obsolete.
     * @hide Since API level {@link android.os.Build.VERSION_CODES#JELLY_BEAN_MR1}
     */
    ...
    /**
     * Gets whether this WebView uses WebView's background instead of internal
     ...
     * has implemented a fully functional Autofill feature to store form data.
     ...
     * 
     * @deprecated Saving passwords in WebView will not be supported in future
     *             versions.
     */
    @Deprecated
    ...
    /**
     * Sets policy for third party cookies. Developers should access this via
     * {@link CookieManager#setShouldAcceptThirdPartyCookies}.
     * 
     * @hide Internal API.
     */
    ...
    /**
     * Gets policy for third party cookies. Developers should access this via
     * {@link CookieManager#getShouldAcceptThirdPartyCookies}.
     * 
     * @hide Internal API
     */
     ...
     * percent using {@link #setTextZoom}, this will return the closest matching
     * {@link TextSize}.
     *
     ...
     * is utilized to display mobile-oriented pages, the desired effect can be
     * achieved by adjusting 'width' and 'initial-scale' attributes of page's 'meta
     * viewport' tag. For pages lacking the tag,
     * {@link android.webkit.WebView#setInitialScale} and
     * {@link #setUseWideViewPort} can be used.
     *
     * @param zoom the zoom density
     * @deprecated This method is no longer supported, see the function
     *             documentation for recommended alternatives.
     */
    @Deprecated
     ...
     * HTML meta tag or should use a wide viewport. When the value of the setting is
     * {@code false}, the layout width is always set to the width of the WebView
     * control in device-independent (CSS) pixels. When the value is {@code true}
     * and the page contains the viewport meta tag, the value of the width specified
     * in the tag is used. If the page does not contain the tag or does not provide
     * a width, then a wide viewport will be used.
     *
    ...
    /**
     * Gets whether the WebView supports the &quot;viewport&quot; HTML meta tag or
     * will use a wide viewport.
     *
     ...
     * {@link WebChromeClient#onCreateWindow} must be implemented by the host
     * application. The default is {@code false}.
     *
     ...
     *
     * @param size a non-negative integer between 1 and 72. Any number outside the
     *             specified range will be pinned.
     */
     ...
     * scheme. Use {@link #setBlockNetworkImage} to control loading only of images
     * specified using network URI schemes. Note that if the value of this setting
     * is changed from {@code false} to {@code true}, all images resources
     * referenced by content currently displayed by the WebView are loaded
     * automatically. The default is {@code true}.
     *
     ...
     * (resources accessed via http and https URI schemes). Note that this method
     * has no effect unless {@link #getLoadsImagesAutomatically} returns
     * {@code true}. Also note that disabling all network loads using
     * {@link #setBlockNetworkLoads} will also prevent network images from loading,
     * even if this flag is set to false. When the value of this setting is changed
     * from {@code true} to {@code false}, network images resources referenced by
     * content currently displayed by the WebView are fetched automatically. The
     ...
     *
     * @return {@code true} if the WebView does not load image resources from the
     *         network
     ...
     * {@link #setBlockNetworkImage} to only avoid loading image resources. Note
     * that if the value of this setting is changed from {@code true} to
     * {@code false}, network resources referenced by content currently displayed by
     * the WebView are not fetched until {@link android.webkit.WebView#reload} is
     * called. If the application does not have the
     * {@link android.Manifest.permission#INTERNET} permission, attempts to set a
     * value of {@code false} will cause a {@link java.lang.SecurityException} to be
     * thrown. The default value is {@code false} if the application has the
     ...
     *
     * @return {@code true} if the WebView does not load any resources from the
     *         network
     * @see #setBlockNetworkLoads
     */
    ...
    /**
     * Tells the WebView to enable JavaScript execution. <b>The default is
     * {@code false}.</b>
     *
     ...
     * allowed to access content from any origin. This includes access to content
     * from other file scheme URLs. See {@link #setAllowFileAccessFromFileURLs}. To
     * enable the most restrictive, and therefore secure policy, this setting should
     * be disabled. Note that this setting affects only JavaScript access to file
     * scheme resources. Other access to such resources, for example, from image
     * HTML elements, is unaffected. To prevent possible violation of same domain
     * policy when targeting
     * {@link android.os.Build.VERSION_CODES#ICE_CREAM_SANDWICH_MR1} and earlier,
     * you should explicitly set this value to {@code false}.
     * <p>
     * The default value is {@code true} for apps targeting
     * {@link android.os.Build.VERSION_CODES#ICE_CREAM_SANDWICH_MR1} and below, and
     * {@code false} when targeting
     * {@link android.os.Build.VERSION_CODES#JELLY_BEAN} and above.
     *
     ...
     * restrictive, and therefore secure, policy this setting should be disabled.
     * Note that the value of this setting is ignored if the value of
     * {@link #getAllowUniversalAccessFromFileURLs} is {@code true}. Note too, that
     * this setting affects only JavaScript access to file scheme resources. Other
     * access to such resources, for example, from image HTML elements, is
     * unaffected. To prevent possible violation of same domain policy when
     * targeting {@link android.os.Build.VERSION_CODES#ICE_CREAM_SANDWICH_MR1} and
     * earlier, you should explicitly set this value to {@code false}.
     * <p>
     * The default value is {@code true} for apps targeting
     * {@link android.os.Build.VERSION_CODES#ICE_CREAM_SANDWICH_MR1} and below, and
     * {@code false} when targeting
    ...
    /**
     * Tells the WebView to enable, disable, or have plugins on demand. On demand
     ...
     * clicked, the plugin will be enabled. The default is {@link PluginState#OFF}.
     *
     ...
     * <ul>
     * <li>an application must have permission to access the device location, see
     ...
     * hole-punching) for videos embedded using HTML &lt;video&gt; tag.<br>
     ...
     * <li>Changes made to position or dimensions of a video element may be
     * propagated to the corresponding video overlay with a noticeable delay.
     * <li>The video overlay is not visible to web APIs and as such may not interact
     ...
     * more memory. The default value is false.<br>
     * Please follow these guidelines to limit memory usage:
     * <ul>
     * <li>WebView size should be not be larger than the device screen size.
     * <li>Limit use of this mode to a small number of WebViews. Use it for visible
     ...
     * <a href="{@docRoot}reference/android/webkit/WebView.html">general Safe
     * Browsing info</a>). The manifest tag has a lower precedence than this API.
     *
     * <p>
