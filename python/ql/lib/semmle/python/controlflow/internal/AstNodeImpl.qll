...
module Ast implements AstSig<Py::Location> {
  ...
  class CatchClause extends Stmt {
     ...
     *
     * This is the type expression (`except TypeError:` → `TypeError`). A
     * bare `except:` has no pattern and is therefore a catch-all that
     * matches any exception. The type test and the variable binding are
     * independent child nodes in Python, matching the shared CFG model.
     */
    AstNode getPattern() { result.asExpr() = handler.getType() }
