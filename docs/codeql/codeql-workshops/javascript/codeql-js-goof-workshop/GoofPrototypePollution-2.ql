 ...
 * @kind path-problem
 */
// The plain query produced only pairs.  Let's check the paths.
...
from PrototypePollConfiguration cfg, DataFlow::PathNode source, DataFlow::PathNode sink
where cfg.hasFlowPath(source, sink)
select sink.getNode(), source, sink, "Path from $@ to $@", source, "source", sink, "sink"
