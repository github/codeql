...
def process_single_file(filename):
    ...
    isFileLevelAnnotated = "overlay[local]" in moduleAnnotations or "overlay[local?]" in moduleAnnotations

    if configuresDataflow or isFileLevelAnnotated:
        ...
        print("file \""+filename+" was annotated using an existing file-level module statment")
        with open(filename, "w") as file_out:
            for line in lines:
                if line.rstrip().endswith("module;"):
                    file_out.write("overlay[local?]\n")
                file_out.write(line)
    elif lines[0].startswith("import") or lines[0].startswith("private"):
        ...
        with open(filename, "w") as file_out:
          ...
          strip_comments("".join(lines)).lstrip().startswith("private")):
