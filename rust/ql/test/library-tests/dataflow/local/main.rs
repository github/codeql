...
fn parse() {
    ...
    sink_string(b); // $ MISSING: hasTaintFlow=90 - we need to extract builtins
    sink(c); // $ MISSING: hasTaintFlow=90 - we need to extract builtins
    sink(d); // $ MISSING: hasTaintFlow=90 - we need to extract builtins
}
...
fn conversions() {
    ...
    sink(i64::from(a)); // $ MISSING: hasValueFlow=50 - we need to extract builtins

    let b: i32 = source(51) as i32;

    sink(b as i64); // $ hasTaintFlow=51
    sink(b.into()); // $ MISSING: hasTaintFlow=51
    sink(i64::from(b)); // $ MISSING: hasTaintFlow=51 - we need to extract builtins
}
