...
impl MyStruct {
    fn get_certificate(&self) -> String {
        return get_string();
    }
    fn get_certificate_url(&self) -> String {
        return get_string();
    }
    fn get_certificate_file(&self) -> String {
        return get_string();
    }
}
...
fn test_credentials(
    ...
    hashkey: &[u64; 4],
    hash_key: &[u64; 4],
    sessionkeypath: &[u64; 4],
    account_key_path: &[u64; 4],
    ms: &MyStruct,
) {
    // credentials
...
fn test_private_info(info: &MyPrivateInfo, details: &ContactDetails) {
    // private info
    ...
    for n in info.medical_notes.iter() {
        sink(n.as_str()); // $ MISSING: sensitive=private -- no type inference for `for` loops
    }
