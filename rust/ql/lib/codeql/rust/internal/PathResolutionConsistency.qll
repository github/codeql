...
query predicate multiplePathResolutions(Path p, ItemNode i) {
  ...
  not p = any(CallExpr pe).getFunction().(PathExpr).getPath() and
...
query predicate multipleCallTargets(ArgsExpr call, Callable target) {
...
int getPathResolutionInconsistencyCounts(string type) {
  ...
  result = count(ArgsExpr call | multipleCallTargets(call, _) | call)
  or
