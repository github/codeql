...
private class AssocFunction extends Function {
  AssocFunction() { not this.hasSelfParam() }
}
  ...
  private Type getCallExprType(CallExpr ce, Path p, Function f, TypePath tp) {
    ...
    result =
      [
        ...
        f.(AssocFunctionCallMatchingInput::Declaration).getReturnType(tp)
      ]
  }
  ...
  HasTypeTreeSig Arg, IsInstantiationOfInputSig<Arg, FunctionType> Input>
{
  ...
  predicate argIsInstanceOf(Arg arg, ImplOrTraitItemNode i, FunctionType constraint) {
    IsInstantiationOf<ArgSubst, FunctionType, IsInstantiationOfInput>::isInstantiationOf(arg, i,
      constraint)
  }

  predicate argIsNotInstanceOf(Arg arg, ImplOrTraitItemNode i, FunctionType constraint) {
...
private module MethodResolution {
  ...
  abstract class MethodCall extends Expr {
    ...
    private predicate hasIncompatibleTarget(ImplOrTraitItemNode i, string derefChainBorrow) {
      ArgIsInstantiationOf<MethodCallCand, ReceiverIsInstanceOfSelfParamInput>::argIsNotInstanceOf(MkMethodCallCand(this,
          derefChainBorrow), i, _)
      or
      IsInstantiationOf<MethodCallCallExpr, TypeMentionTypeTree, TypeQualifierIsInstanceOfImplSelfInput>::isNotInstantiationOf(this,
        i, _) and
      derefChainBorrow = ";"
    }
  ...
  private class MethodCallCand extends MkMethodCallCand {
    ...
    private predicate hasIncompatibleInherentTarget(Impl impl) {
      ArgIsInstantiationOf<MethodCallCand, ReceiverIsNotInstanceOfInherentSelfParamInput>::argIsNotInstanceOf(this,
        impl, _)
    }
    ...
    pragma[nomagic]
    private predicate typeQualifierIsInstanceOf(ImplOrTraitItemNode i) {
      IsInstantiationOf<MethodCallCallExpr, TypeMentionTypeTree, TypeQualifierIsInstanceOfImplSelfInput>::isInstantiationOf(mc_,
        i, _)
    }
    ...
    private predicate argIsInstanceOf(ImplOrTraitItemNode i, string name, int arity) {
      (
        ArgIsInstantiationOf<MethodCallCand, ReceiverIsInstanceOfSelfParamInput>::argIsInstanceOf(this,
          i, _)
        or
        this.typeQualifierIsInstanceOf(i)
      ) and
    ...
    private Type inferPositionalArgumentType(FunctionTypePosition pos, TypePath path) {
      ...
      result = substituteLookupTraits(inferType(mc_.getArgument(pos.asArgumentPosition()), path))
    }

    pragma[nomagic]
    private Method resolveAmbigousCallTargetCand(FunctionTypePosition pos, TypePath path, Type type) {
   ...
   * a `self` parameter.
   */
  private module ReceiverIsInstanceOfSelfParamInput implements
   ...
   */
  private module TypeQualifierIsInstanceOfImplSelfInput implements
   ...
   */
  private module ReceiverIsNotInstanceOfInherentSelfParamInput implements
    IsInstantiationOfInputSig<MethodCallCand, FunctionType>
  {
    pragma[nomagic]
    predicate potentialInstantiationOf(
      ...
      ReceiverIsInstanceOfSelfParamInput::potentialInstantiationOf0(mcc, abs, constraint) and
...
private module AssocFunctionResolution {
  ...
  private predicate traitFunctionDependsOnArgument(
    TraitItemNode trait, AssocFunction traitFunction, FunctionTypePosition pos,
    AssocFunction implFunction, TypePath path, Type type
  ) {
  ...
  private predicate functionInfoBlanketRelevantPos(
    AssocFunction f, string name, int arity, ImplItemNode impl, Trait trait,
  ...
  private predicate blanketCallTraitCandidate(Element fc, Trait trait) {
    exists(string name, int arity |
      fc.(AssocFunctionCall).hasNameAndArity(name, arity) and
  ...
  /** An associated function call tagged with a position. */
  class AssocFunctionCallAndPos extends MkAssocFunctionCallAndPos {
    AssocFunctionCall fc;
    FunctionTypePosition pos;

    AssocFunctionCallAndPos() { this = MkAssocFunctionCallAndPos(fc, pos) }
  ...
  private module ArgSatisfiesBlanketConstraintInput implements
    BlanketImplementation::SatisfiesBlanketConstraintInputSig<AssocFunctionCallAndPos>
  {
    pragma[nomagic]
    predicate hasBlanketCandidate(
   ...
   */
  private module ArgIsInstanceOfBlanketParamInput implements
    IsInstantiationOfInputSig<AssocFunctionCallAndPos, FunctionType>
  {
    pragma[nomagic]
    predicate potentialInstantiationOf(
      ...
      exists(FunctionTypePosition pos |
        BlanketImplementation::SatisfiesBlanketConstraint<AssocFunctionCallAndPos, ArgSatisfiesBlanketConstraintInput>::satisfiesBlanketConstraint(fcp,
          abs) and
  ...
  /** An associated function call, `f(x)`. */
  final class AssocFunctionCall extends CallExpr {
    AssocFunctionCall() {
      // even if a function cannot be resolved by path resolution, it may still
      // be possible to resolve a blanket implementation
      forall(Function f | f = CallExprImpl::getResolvedFunction(this) | f instanceof AssocFunction)
    }
    ...
    /**
     * Gets the blanket function that this call resolves to, if any.
     */
    pragma[nomagic]
    private ItemNode resolveCallTargetCand() {
      not this.(Call).hasTrait() and
      result = this.getPathResolutionResolved()
      or
      exists(ImplItemNode impl, string name |
        this.hasNameAndArity(name, _) and
        ArgIsInstantiationOf<AssocFunctionCallAndPos, ArgIsInstanceOfBlanketParamInput>::argIsInstanceOf(MkAssocFunctionCallAndPos(this,
            _), impl, _) and
        result = impl.getASuccessor(name)
      )
    }

    pragma[nomagic]
    private AssocFunction resolveAmbigousCallTargetCand(
      FunctionTypePosition pos, TypePath path, Type type
    ) {
      result = this.resolveCallTargetCand() and
    ...
    private Type inferArgumentType(FunctionTypePosition pos, TypePath path) {
      result = substituteLookupTraits(inferType(this.getNodeAt(pos), path))
    }

    pragma[nomagic]
    private predicate hasTraitResolved(
      TraitItemNode trait, AssocFunction resolved, FunctionTypePosition pos, TypePath path,
      Type type
    ) {
    ...
    ItemNode resolveCallTarget() {
      ...
      exists(FunctionTypePosition pos, TypePath path, Type type |
        ...
        or
        // E.g. `Default::default()`
        exists(TraitItemNode trait, AssocFunction resolved |
          this.hasTraitResolved(trait, resolved, pos, path, type) and
          traitFunctionDependsOnArgument(trait, resolved, pos, result, path, type)
        )
      )
    }
  }
...
/**
 * A matching configuration for resolving types of associated function calls
 * like `foo::bar(baz)`.
 */
private module AssocFunctionCallMatchingInput implements MatchingInputSig {
  import FunctionTypePositionMatchingInput
  ...
  private class AssocFunctionDecl extends Declaration, AssocFunction instanceof MethodCallMatchingInput::Declaration
  {
    override TypeParameter getTypeParameter(TypeParameterPosition ppos) {
...
private module AssocFunctionCallMatching = Matching<AssocFunctionCallMatchingInput>;

pragma[nomagic]
private Type inferAssocFunctionCallType(AstNode n, TypePath path) {
  exists(
    AssocFunctionCallMatchingInput::Access a, AssocFunctionCallMatchingInput::AccessPosition apos
  |
    n = a.getNodeAt(apos) and
    result = AssocFunctionCallMatching::inferAccessType(a, apos, path)
  )
}
...
private module TupleStructPatMatchingInput implements MatchingInputSig {
  ...
  class Declaration = AssocFunctionCallMatchingInput::TupleDeclaration;
...
private module Cached {
  ...
  Type inferType(AstNode n, TypePath path) {
    ...
    (
      ...
      or
      result = inferAssocFunctionCallType(n, path)
      or
...
private module Debug {
  ...
  predicate debugInferAssocFunctionCallType(AstNode n, TypePath path, Type t) {
    n = getRelevantLocatable() and
    t = inferAssocFunctionCallType(n, path)
  }
