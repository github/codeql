 ...
 *
 * ```rust
 * let x = ...; // `x` is only a variable if it does not resolve to a constructor
 * ```
 *
 ...
 *   identifier pattern resolves to a constructor, since if it does, it cannot be
 *   shadowed by a variable. We expose this as the predicate `identPatIsResolvable`.
