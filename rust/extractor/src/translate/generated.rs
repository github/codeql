...
impl Translator<'_> {
    ...
    pub(crate) fn emit_assoc_item(&mut self, node: &ast::AssocItem) -> Label<generated::AssocItem> {
        let label = match node {
            ...
            ast::AssocItem::MacroCall(ref inner) => self.emit_macro_call(inner).into(),
            ast::AssocItem::TypeAlias(ref inner) => self.emit_type_alias(inner).into(),
        };
    ...
    pub(crate) fn emit_expr(&mut self, node: &ast::Expr) -> Label<generated::Expr> {
        let label = match node {
            ...
            ast::Expr::AwaitExpr(ref inner) => self.emit_await_expr(inner).into(),
            ast::Expr::BecomeExpr(ref inner) => self.emit_become_expr(inner).into(),
            ast::Expr::BinExpr(ref inner) => self.emit_bin_expr(inner).into(),
            ast::Expr::BlockExpr(ref inner) => self.emit_block_expr(inner).into(),
            ast::Expr::BreakExpr(ref inner) => self.emit_break_expr(inner).into(),
            ast::Expr::CallExpr(ref inner) => self.emit_call_expr(inner).into(),
            ast::Expr::CastExpr(ref inner) => self.emit_cast_expr(inner).into(),
            ast::Expr::ClosureExpr(ref inner) => self.emit_closure_expr(inner).into(),
            ast::Expr::ContinueExpr(ref inner) => self.emit_continue_expr(inner).into(),
            ast::Expr::FieldExpr(ref inner) => self.emit_field_expr(inner).into(),
            ast::Expr::ForExpr(ref inner) => self.emit_for_expr(inner).into(),
            ast::Expr::FormatArgsExpr(ref inner) => self.emit_format_args_expr(inner).into(),
            ast::Expr::IfExpr(ref inner) => self.emit_if_expr(inner).into(),
            ast::Expr::IndexExpr(ref inner) => self.emit_index_expr(inner).into(),
            ast::Expr::LetExpr(ref inner) => self.emit_let_expr(inner).into(),
            ast::Expr::Literal(ref inner) => self.emit_literal(inner).into(),
            ast::Expr::LoopExpr(ref inner) => self.emit_loop_expr(inner).into(),
            ast::Expr::MacroExpr(ref inner) => self.emit_macro_expr(inner).into(),
            ast::Expr::MatchExpr(ref inner) => self.emit_match_expr(inner).into(),
            ast::Expr::MethodCallExpr(ref inner) => self.emit_method_call_expr(inner).into(),
            ast::Expr::OffsetOfExpr(ref inner) => self.emit_offset_of_expr(inner).into(),
            ast::Expr::ParenExpr(ref inner) => self.emit_paren_expr(inner).into(),
            ast::Expr::PathExpr(ref inner) => self.emit_path_expr(inner).into(),
            ast::Expr::PrefixExpr(ref inner) => self.emit_prefix_expr(inner).into(),
            ast::Expr::RangeExpr(ref inner) => self.emit_range_expr(inner).into(),
            ast::Expr::RecordExpr(ref inner) => self.emit_record_expr(inner).into(),
            ast::Expr::RefExpr(ref inner) => self.emit_ref_expr(inner).into(),
            ast::Expr::ReturnExpr(ref inner) => self.emit_return_expr(inner).into(),
            ast::Expr::TryExpr(ref inner) => self.emit_try_expr(inner).into(),
            ast::Expr::TupleExpr(ref inner) => self.emit_tuple_expr(inner).into(),
            ast::Expr::UnderscoreExpr(ref inner) => self.emit_underscore_expr(inner).into(),
            ast::Expr::WhileExpr(ref inner) => self.emit_while_expr(inner).into(),
            ast::Expr::YeetExpr(ref inner) => self.emit_yeet_expr(inner).into(),
            ast::Expr::YieldExpr(ref inner) => self.emit_yield_expr(inner).into(),
        };
    ...
    pub(crate) fn emit_extern_item(&mut self, node: &ast::ExternItem) -> Label<generated::ExternItem> {
        let label = match node {
            ...
            ast::ExternItem::Static(ref inner) => self.emit_static(inner).into(),
            ast::ExternItem::TypeAlias(ref inner) => self.emit_type_alias(inner).into(),
        };
    ...
    pub(crate) fn emit_generic_arg(&mut self, node: &ast::GenericArg) -> Label<generated::GenericArg> {
        let label = match node {
            ...
            ast::GenericArg::LifetimeArg(ref inner) => self.emit_lifetime_arg(inner).into(),
            ast::GenericArg::TypeArg(ref inner) => self.emit_type_arg(inner).into(),
        };
    ...
    pub(crate) fn emit_generic_param(&mut self, node: &ast::GenericParam) -> Label<generated::GenericParam> {
        let label = match node {
            ...
            ast::GenericParam::TypeParam(ref inner) => self.emit_type_param(inner).into(),
        };
    ...
    pub(crate) fn emit_pat(&mut self, node: &ast::Pat) -> Label<generated::Pat> {
        let label = match node {
            ...
            ast::Pat::IdentPat(ref inner) => self.emit_ident_pat(inner).into(),
            ast::Pat::LiteralPat(ref inner) => self.emit_literal_pat(inner).into(),
            ast::Pat::MacroPat(ref inner) => self.emit_macro_pat(inner).into(),
            ast::Pat::OrPat(ref inner) => self.emit_or_pat(inner).into(),
            ast::Pat::ParenPat(ref inner) => self.emit_paren_pat(inner).into(),
            ast::Pat::PathPat(ref inner) => self.emit_path_pat(inner).into(),
            ast::Pat::RangePat(ref inner) => self.emit_range_pat(inner).into(),
            ast::Pat::RecordPat(ref inner) => self.emit_record_pat(inner).into(),
            ast::Pat::RefPat(ref inner) => self.emit_ref_pat(inner).into(),
            ast::Pat::RestPat(ref inner) => self.emit_rest_pat(inner).into(),
            ast::Pat::SlicePat(ref inner) => self.emit_slice_pat(inner).into(),
            ast::Pat::TuplePat(ref inner) => self.emit_tuple_pat(inner).into(),
            ast::Pat::TupleStructPat(ref inner) => self.emit_tuple_struct_pat(inner).into(),
            ast::Pat::WildcardPat(ref inner) => self.emit_wildcard_pat(inner).into(),
        };
    ...
    pub(crate) fn emit_stmt(&mut self, node: &ast::Stmt) -> Label<generated::Stmt> {
        let label = match node {
            ...
            ast::Stmt::LetStmt(ref inner) => self.emit_let_stmt(inner).into(),
        };
    ...
    pub(crate) fn emit_type(&mut self, node: &ast::Type) -> Label<generated::TypeRef> {
        let label = match node {
            ...
            ast::Type::FnPtrType(ref inner) => self.emit_fn_ptr_type(inner).into(),
            ast::Type::ForType(ref inner) => self.emit_for_type(inner).into(),
            ast::Type::ImplTraitType(ref inner) => self.emit_impl_trait_type(inner).into(),
            ast::Type::InferType(ref inner) => self.emit_infer_type(inner).into(),
            ast::Type::MacroType(ref inner) => self.emit_macro_type(inner).into(),
            ast::Type::NeverType(ref inner) => self.emit_never_type(inner).into(),
            ast::Type::ParenType(ref inner) => self.emit_paren_type(inner).into(),
            ast::Type::PathType(ref inner) => self.emit_path_type(inner).into(),
            ast::Type::PtrType(ref inner) => self.emit_ptr_type(inner).into(),
            ast::Type::RefType(ref inner) => self.emit_ref_type(inner).into(),
            ast::Type::SliceType(ref inner) => self.emit_slice_type(inner).into(),
            ast::Type::TupleType(ref inner) => self.emit_tuple_type(inner).into(),
        };
    ...
    pub(crate) fn emit_item(&mut self, node: &ast::Item) -> Label<generated::Item> {
        let label = match node {
            ...
            ast::Item::ExternBlock(ref inner) => self.emit_extern_block(inner).into(),
            ast::Item::ExternCrate(ref inner) => self.emit_extern_crate(inner).into(),
            ast::Item::Fn(ref inner) => self.emit_fn(inner).into(),
            ast::Item::Impl(ref inner) => self.emit_impl(inner).into(),
            ast::Item::MacroCall(ref inner) => self.emit_macro_call(inner).into(),
            ast::Item::MacroDef(ref inner) => self.emit_macro_def(inner).into(),
            ast::Item::MacroRules(ref inner) => self.emit_macro_rules(inner).into(),
            ast::Item::Module(ref inner) => self.emit_module(inner).into(),
            ast::Item::Static(ref inner) => self.emit_static(inner).into(),
            ast::Item::Struct(ref inner) => self.emit_struct(inner).into(),
            ast::Item::Trait(ref inner) => self.emit_trait(inner).into(),
            ast::Item::TraitAlias(ref inner) => self.emit_trait_alias(inner).into(),
            ast::Item::TypeAlias(ref inner) => self.emit_type_alias(inner).into(),
            ast::Item::Union(ref inner) => self.emit_union(inner).into(),
            ast::Item::Use(ref inner) => self.emit_use(inner).into(),
        };
