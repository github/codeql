...
impl Translator<'_> {
    ...
    pub(crate) fn emit_asm_operand(
        ...
        emit_detached!(AsmOperand, self, node, label);
        Some(label)
    }
    pub(crate) fn emit_asm_piece(
        ...
        emit_detached!(AsmPiece, self, node, label);
        Some(label)
    }
    ...
    pub(crate) fn emit_type(&mut self, node: &ast::Type) -> Option<Label<generated::TypeRepr>> {
        ...
        emit_detached!(TypeRepr, self, node, label);
        Some(label)
    }
    pub(crate) fn emit_use_bound_generic_arg(
        ...
        emit_detached!(UseBoundGenericArg, self, node, label);
        Some(label)
    }
