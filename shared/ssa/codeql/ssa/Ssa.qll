...
module Make<LocationSig Location, InputSig<Location> Input> {
  ...
  signature module DataFlowIntegrationInputSig {
    ...
    /** Holds is SSA definition `def` assigns `value` to the underlying variable. */
  ...
  /**
   * Constructs the type `Node` and associated value step relation, which are
   * intended to be included in the `DataFlow::Node` type and local step relation.
   *
  ...
  module DataFlowIntegration<DataFlowIntegrationInputSig DfInput> {
    ...
    predicate localFlowStep(DefinitionExt def, Node nodeFrom, Node nodeTo) {
      (
      ...
      inputFromDef(def, nodeFrom, nodeTo)
      or
      // Flow into phi (read) SSA definition node from (post-update) read
      inputFromRead(def, [nodeFrom, nodeFrom.(ExprPostUpdateNode).getPreUpdateNode()], nodeTo)
      or
