...
module TestPostProcessing {
  ...
  module Make<InlineExpectationsTestSig Input> {
    ...
    module TestImpl2 implements InlineExpectationsTestSig {
      ...
      bindingset[relativePath]
      string getStartCommentMarker(string relativePath) {
        result = Input::getStartCommentMarker(relativePath)
      }

      bindingset[relativePath]
      string getEndCommentMarker(string relativePath) {
        result = Input::getEndCommentMarker(relativePath)
      }
    ...
    private module LearnEditsImpl {
      ...
      private string wrapInComment(string relativePath, string content) {
        exists(string startMarker |
          startMarker = Input::getStartCommentMarker(relativePath) and
          if exists(Input::getEndCommentMarker(relativePath))
          then result = startMarker + " " + content + " " + Input::getEndCommentMarker(relativePath)
      ...
      /**
       * Gets the exclusive end column for replacing `comment`. Same-line CodeQL locations use
       * inclusive end columns, so their end boundary is one column later. A comment whose location
       * ends on another line uses 0, the engine's replace-to-EOL sentinel; this covers extractors
      ...
      private int getReplacementEndColumn(TestImpl2::ExpectationComment comment) {
        exists(string relativePath, int sl, int el, int ec |
          parseLocationString(comment.getLocation().getRelativeUrl(), relativePath, sl, _, el, ec) and
          if sl = el then result = ec + 1 else result = 0
        )
      }
       ...
       * - `"replace"`: replace the 1-based half-open column range `[startColumn, endColumn)` with
       *   `text`. Equal boundaries insert `text`; the empty string deletes a non-empty range.
       *   `endColumn = 0` extends the range to the end of the line.
       *
      ...
      query predicate learnEdits(
        ...
        // `MISSING:` columns. The comment's inclusive location end is converted to the protocol's
        // exclusive end boundary when it is on the same line. A multi-line location uses
        // `endColumn = 0`, the engine's to-EOL convention, because some extractors (e.g. Swift)
        // report a line comment as ending at column 1 of the next line. The replacement contains the
        // re-rendered desired expectations (keeping any trailing regular comment), just the trailing
        // regular comment when no expectation remains but a note like `// $ Alert // note` does, or
        // the empty string when nothing remains. An empty to-EOL replacement also trims the
        // whitespace gap left by the removed comment.
        exists(TestImpl2::ExpectationComment comment, string relativePath, int sl, int sc |
          ...
          startColumn = sc and
          endColumn = getReplacementEndColumn(comment) and
          (
