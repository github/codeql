  ...
  InputSig<Location, Cfg::ControlFlowNode, Cfg::BasicBlock> Input>
{
  ...
  module Flow<ConfigSig Config> {
    ...
    private predicate sourceReachesBlock(
      ...
      |
        exists(boolean abrupt | entersFinally(midbb, bb, abrupt, _) and fs = midfs.enter(abrupt))
        or
        exists(boolean abrupt | leavesFinally(midbb, bb, abrupt, _) and fs = midfs.leave(abrupt))
        or
        not entersFinally(midbb, bb, _, _) and not leavesFinally(midbb, bb, _, _) and fs = midfs
      )
    }
    ...
    private predicate sourceReachesBlockWithTrackedVar(
      SsaDefinition srcDef, BasicBlock srcBb, SsaDefinition def, BasicBlock bb,
      ...
      or
      exists(
        SourceVariable src, SsaDefinition middef, BasicBlock midbb, SsaDefOption tracked0,
        GuardValueExt val0
      |
        sourceReachesBlockWithTrackedVar(srcDef, srcBb, middef, midbb, tracked0, val0, var) and
    ...
    predicate flow(
      ...
      exists(BasicBlock srcBb, BasicBlock sinkBb, SourceVariable srcVar |
        ...
        forall(SourceVariable t | relevantSplit(srcVar, t) |
          sourceReachesBlockWithTrackedVar(srcDef, srcBb, sinkDef, sinkBb, _, _, t)
        )
      )
    }
    ...
    predicate escapeFlow(ControlFlowNode src, SsaDefinition srcDef) {
      ...
      exists(BasicBlock srcBb, SsaDefinition escDef, BasicBlock escBb, SourceVariable srcVar |
        ...
        srcVar = srcDef.getSourceVariable() and
        forall(SourceVariable t | relevantSplit(srcVar, t) |
          sourceReachesBlockWithTrackedVar(srcDef, srcBb, escDef, escBb, _, _, t)
        )
      )
    }
  }
