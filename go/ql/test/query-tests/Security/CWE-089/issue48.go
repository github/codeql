...
func handler1(db *sql.DB, req *http.Request) {
	...
	b, _ := ioutil.ReadAll(req.Body) // $ Source[go/sql-injection]
	...
	db.Query(q3) // $ Alert[go/sql-injection] // NOT OK
	...
	b2, _ := ioutil.ReadAll(req.Body) // $ Source[go/sql-injection]
