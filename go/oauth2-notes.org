...
-do bare bones CallExpr select - you can be quick about this because they've seenit before

- add where clause - demo the chaining of the predicates. i.e .getTarget() returns Function but we want string "HasPrefix" so we . again and go through member predicates 

- expalin that strings.HasPrefix checkes that a given string argumenet begins with a certain prefix 
