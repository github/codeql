...
abstract class BadDynamicCall extends DynamicExpr {
  ...
  Type possibleBadTypeForRelevantSource(Variable v, int i, Expr source) {
    exists(Type t |
      t = possibleTypeForRelevantSource(v, i, source) |
    ...
    and not result instanceof Interface
    and not result.(Class).isAbstract()
    and not result instanceof NullType
    and not result instanceof DynamicType
  }
...
class BadDynamicMethodCall extends BadDynamicCall, DynamicMethodCall {
  ...
  override predicate isBad(Variable v, ValueOrRefType pt, Expr pts, string message, string target) {
    ...
    and
    message = "The $@ of this dynamic method invocation can obtain (from $@) type $@, which does not have a method '" + getLateBoundTargetName() + "' with the appropriate signature."
    and
    target = "target"
  }
}
...
class BadDynamicOperatorCall extends BadDynamicCall, DynamicOperatorCall {
  override AssignableRead getARelevantVariableAccess(int i) {
    result = getRuntimeArgument(i)
  }

  override predicate isBad(Variable v, ValueOrRefType pt, Expr pts, string message, string target) {
    exists(int i |
      pt = possibleBadTypeForRelevantSource(v, i, pts)
      and
      not pt.containsTypeParameters()
      and
      not exists(Operator o, Type paramType |
        paramType = getADynamicParameterType(o, i) |
    ...
    and
    message = "The $@ of this dynamic operator can obtain (from $@) type $@, which does not match an operator '" + getLateBoundTargetName() + "' with the appropriate signature."
  }
