...
where
  ...
  and asType = e.getTargetType()
  and not(asType.isSealed())
  and not exists(MethodCall c, Variable v |
    c.getEnclosingCallable() = m
    and c.getTarget().getName() = "GetType"
    and v = c.getQualifier().(VariableAccess).getTarget() |
    v = m.getParameter(0) or
    v.getAnAssignedValue() = e
  )
select e, m.getDeclaringType().getName() + ".Equals(object) should not use \"as\" on its parameter, as it will not work properly for subclasses of " + asType.getName() + "."
