  ...
  exists(FieldOrProperty f |
    ...
    (
      ...
      exists(ContentList cl |
        FrameworkDataFlowImpl::Compilation::summary(_, _, cl, _, _, _) and
...
private predicate fieldOrPropertyRead(Expr e1, Content c, FieldOrPropertyRead e2) {
  ...
  exists(FieldOrProperty ret | c = ret.getContent() |
    ...
    exists(ContentList cl, Property target |
      FrameworkDataFlowImpl::Compilation::summary(_, _, _, _, cl, _) and
...
private module Cached {
  private import FrameworkDataFlowSpecific as FrameworkDataFlowSpecific
  ...
  newtype TNode =
    ...
    TSummaryParameterNode(FrameworkCallable c, int i) {
      exists(SummaryInput input |
        FrameworkDataFlowImpl::Compilation::summary(c, input, _, _, _, _)
      |
    ...
    TSummaryInternalNode(
      ...
      FrameworkDataFlowImpl::Compilation::internalNodeRange(c, input, inputContents, output,
        outputContents, preservesValue, state)
    } or
    TSummaryReturnNode(FrameworkCallable c, ReturnKind rk) {
      exists(SummaryOutput output |
        FrameworkDataFlowImpl::Compilation::summary(c, _, _, output, _, _) and
        rk = FrameworkDataFlowSpecific::Private::toReturnKind(output)
      )
    } or
    TSummaryDelegateOutNode(FrameworkCallable c, int pos) {
      exists(SummaryInput input |
        FrameworkDataFlowImpl::Compilation::summary(c, input, _, _, _, _) and
        input = SummaryInput::delegate(pos)
      )
    } or
    TSummaryDelegateArgumentNode(FrameworkCallable c, int delegateIndex, int parameterIndex) {
      exists(SummaryOutput output |
        FrameworkDataFlowImpl::Compilation::summary(c, _, _, output, _, _) and
  ...
  predicate simpleLocalFlowStep(Node nodeFrom, Node nodeTo) {
    ...
    or
    FrameworkDataFlowImpl::Compilation::localStep(nodeFrom, nodeTo, true)
    or
  ...
  predicate localFlowStepImpl(Node nodeFrom, Node nodeTo) {
    ...
    FrameworkDataFlowImpl::Compilation::throughStep(nodeFrom, nodeTo, true)
  }
  ...
  predicate storeStepImpl(Node node1, Content c, Node node2) {
    ...
    or
    FrameworkDataFlowImpl::Compilation::storeStep(node1, c, node2)
  }
  ...
  predicate readStepImpl(Node node1, Content c, Node node2) {
    ...
    or
    FrameworkDataFlowImpl::Compilation::readStep(node1, c, node2)
  }
  ...
  predicate clearsContent(Node n, Content c) {
    ...
    or
    FrameworkDataFlowImpl::Compilation::storeStep(n, c, _) and
    not c instanceof ElementContent
    or
    exists(SummaryInput input, DataFlowCall call, int i |
      FrameworkDataFlowImpl::Compilation::clearsContent(input, call, c) and
...
private class SummaryInternalNode extends SummaryNodeImpl, TSummaryInternalNode {
  ...
  private ContentList inputContents;
  private SummaryOutput output;
  private ContentList outputContents;
  private boolean preservesValue;
  private FrameworkDataFlowImpl::Compilation::SummaryInternalNodeState state;

  SummaryInternalNode() {
    this =
      TSummaryInternalNode(c, input, inputContents, output, outputContents, preservesValue, state)
  }
  ...
  override DataFlowType getDataFlowType() {
    result =
      FrameworkDataFlowImpl::Compilation::internalNodeType(c, input, inputContents, output,
        outputContents, preservesValue, state)
  }
