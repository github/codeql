...
private module Cached {
  ...
  newtype TReturnPosition =
    TNormalReturnPosition(DotNet::Callable c) { c.canReturn(_) } or
    TYieldReturnPosition(Callable c) { c.canYieldReturn(_) } or
    TOutRefReturnPosition(Parameter p) { callableReturnsOutOrRef(_, p, _) } or
    TImplicitCapturedReturnPosition(Ssa::SourceVariable v) {
  ...
  newtype TDataFlowCall =
    TCSharpCall(ControlFlow::Nodes::ElementNode cfn, DispatchCall dc) {
  ...
  cached
  DotNet::Callable viableCallable(DataFlowCall call) { result = call.getARuntimeTarget() }
  ...
  private DotNet::Callable viableImplInCallContext(DataFlowCall call, DataFlowCall ctx) {
    exists(CallContext cc | result = viableDelegateCallable(call, cc) |
      cc.(ArgumentCallContext).isArgument(ctx.getExpr(), _)
    )
  }
  ...
  /**
   * Gets a viable run-time target for the call `call` in the context
   * `ctx`. This is restricted to those call nodes and results which the
   * return flow from the result to `call` restricts the possible context
   * `ctx`.
   */
  cached
  ...
  OutNode getAnOutputAtCall(DataFlowCall call, ReturnPosition pos) {
    exists(DotNet::Callable c | pos.getCallable() = c |
      c = viableCallable(call) and
      not call instanceof TransitiveCapturedDataFlowCall and
      (
        // normal `return`
        result = call.getNode() and
        pos instanceof NormalReturnPosition
        or
        // `yield return`
        result = call.getNode() and
        pos instanceof YieldReturnPosition
        or
        // `out`/`ref` parameter
        exists(Parameter p, AssignableDefinitions::OutRefDefinition def |
          p.getSourceDeclaration() = pos.(OutRefReturnPosition).getParameter()
        |
          def = result
                .(SsaDefinitionNode)
                .getDefinition()
                .(Ssa::ExplicitDefinition)
                .getADefinition() and
          def.getTargetAccess() = call.getExpr().(Call).getArgumentForParameter(p)
        )
        or
        // implicit captured variable return (single call)
        result = pos.(ImplicitCapturedReturnPosition).getAnOutNode()
      )
      or
      // implicit captured variable return (transitive call chain)
      call.(TransitiveCapturedDataFlowCall).getARuntimeTarget() = pos.getCallable() and
      result = pos.(ImplicitCapturedReturnPosition).getAnOutNode()
    )
  }
}
 ...
 */
abstract class ReturnPosition extends TReturnPosition {
  /** Gets the callable that a value can be returned from. */
  abstract DotNet::Callable getCallable();

  /** Gets a return node that can return a value at this position. */
  abstract ReturnNode getAReturnNode();
 ...
 * body.
 */
class NormalReturnPosition extends ReturnPosition, TNormalReturnPosition {
  private DotNet::Callable c;

  NormalReturnPosition() { this = TNormalReturnPosition(c) }

  override DotNet::Callable getCallable() { result = c }

  override ExprReturnNode getAReturnNode() { c.canReturn(result.getExpr()) }

  override string toString() { result = "return " + c }
}

/** A value returned from a callable using a `yield return` statement. */
class YieldReturnPosition extends ReturnPosition, TYieldReturnPosition {
  private Callable c;

  YieldReturnPosition() { this = TYieldReturnPosition(c) }

  override Callable getCallable() { result = c }

  override TaintedReturnNode getAReturnNode() {
    c.canYieldReturn(result.getUnderlyingNode().getExpr())
  }

  override string toString() { result = "yield return " + c }
}

/** A value returned from a callable using an `out` or a `ref` parameter. */
class OutRefReturnPosition extends ReturnPosition, TOutRefReturnPosition {
  private Parameter p;

  OutRefReturnPosition() { this = TOutRefReturnPosition(p) }

  /** Gets the relevant `out` or `ref` parameter. */
  Parameter getParameter() { result = p }

  override DotNet::Callable getCallable() { result = p.getCallable() }

  override OutRefReturnNode getAReturnNode() {
    result.getDefinition().getSourceVariable().getAssignable() = p
  }

  override string toString() { result = "return (out/ref) " + p }
}

/** A value implicitly returned from a callable using a captured variable. */
class ImplicitCapturedReturnPosition extends ReturnPosition, TImplicitCapturedReturnPosition {
  private Ssa::SourceVariable v;

  ImplicitCapturedReturnPosition() { this = TImplicitCapturedReturnPosition(v) }

  /** Gets a node that can read a value returned at this position. */
  SsaDefinitionNode getAnOutNode() {
    exists(Ssa::ExplicitDefinition def |
      def.isCapturedVariableDefinitionFlowOut(result.getDefinition(), _)
    |
      v = def.getSourceVariable()
    )
  }

  override ImplicitCapturedReturnNode getAReturnNode() {
    result.getDefinition().getSourceVariable() = v
  }

  override DotNet::Callable getCallable() { result = v.getEnclosingCallable() }

  override string toString() { result = "return (captured) " + v }
}
...
abstract class DataFlowCall extends TDataFlowCall {
  ...
  /** Gets the control flow where this call happens, if any. */
...
/** A non-delegate call relevant for data flow. */
abstract class NonDelegateDataFlowCall extends DataFlowCall { }

/** A non-delegate C# call relevant for data flow. */
class CSharpDataFlowCall extends NonDelegateDataFlowCall, TCSharpCall {
  private ControlFlow::Nodes::ElementNode cfn;

  private DispatchCall dc;

  CSharpDataFlowCall() { this = TCSharpCall(cfn, dc) }
...
/** A non-delegate CIL call relevant for data flow. */
class CilDataFlowCall extends NonDelegateDataFlowCall, TCilCall {
...
/**
 * An implicit delegate call relevant for data flow. For example, the implicit
...
class ImplicitDelegateDataFlowCall extends DelegateDataFlowCall, TImplicitDelegateCall {
   ...
   * call `cn` targeting a library callable.
   */
