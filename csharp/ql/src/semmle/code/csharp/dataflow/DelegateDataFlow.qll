...
private predicate flowsFrom(
  ...
  exists(DataFlow::Node mid |
    ...
    isReturned = true and
    flowOutOfNonDelegateCall(node.(ReturnNode).getPosition(), mid)
  )
  or
  ...
  exists(DataFlow::ExprNode mid |
    flowsFrom(sink, mid, _, _) and
    isReturned = true and
    flowOutOfDelegateCall(mid.getExpr(), node.(ReturnNode).getPosition(), lastCall)
  )
}
...
pragma[noinline]
private predicate flowOutOfNonDelegateCall(NormalReturnPosition pos, ExprNode out) {
  exists(NonDelegateCall call |
    call.getARuntimeTarget() = pos.getCallable() and
    out.getExpr() = call
  )
}
