...
predicate accessesLocalFieldOrProperty(Method m, Declaration f) {
  (exists(FieldAccess fa | fa.getEnclosingCallable() = m and fa.getTarget() = f)
   or
   exists(PropertyAccess pa | pa.getEnclosingCallable() = m and pa.getTarget() = f)) and
...
predicate lackOfCohesionHS(/* this */ ValueOrRefType t, float locm)  {
  exists(int m, float r |
     ...
     m = count(Method method | hasAccessingMethod(t, method))
     and
...
predicate ignoreLackOfCohesionCK(Method m) {
  none()
}
...
predicate lackOfCohesionCK(/* this */ ValueOrRefType t, float locm)  {
  exists(int methods, int linked, float n |
    methods = count(Method m | t = m.getDeclaringType() and not ignoreLackOfCohesionCK(m))
    and
    linked = count(Method m1, Method m2 | shareFieldOrProperty(t, m1, m2))
    and
    ...
    // is C, is (C - 1) * C.
    n = ((methods - 1) * methods) / 2.0 - linked
    and
    (
      n < 0 and locm = 0
      or
