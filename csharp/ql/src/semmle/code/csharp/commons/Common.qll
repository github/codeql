...
predicate isCreatingObject(ObjectCreation oc, string qualifiedName) {
  exists(RefType t | t = oc.getType() |
    t.getBaseClass*().hasQualifiedName(qualifiedName)
  )
}
...
predicate isReturningObject(MethodCall mc, string qualifiedName) {
  ...
  exists (RefType t | t = mc.getType() |
    t.hasQualifiedName(qualifiedName)
  )
}
...
predicate isMethodCalledWithArg(MethodCall mc, string qualifiedName, string methodName, int argumentIndex, string argumentValue) {
  mc.getTarget().fromLibrary() and 
