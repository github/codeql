...
module HardcodedCredentials {
  ...
  class NonEmptyStringLiteral extends Source {
    NonEmptyStringLiteral() {
      this.getExpr().(StringLiteral).getValue().length() > 1
    }
  }
   ...
   */
  private predicate getCredentialSink(Expr sink, string sinkName, Element supplementaryElement, string description) {
  ...
  class HardcodedCredentialsSinkExpr extends Sink {
    ...
    override
    Element getSupplementaryElement() {
      result = supplementaryElement
    }
  ...
  class MembershipUserUserNameSink extends Sink {
    ...
    MembershipUserUserNameSink() {
      call.getTarget().getDeclaringType().getABaseType*() instanceof SystemWebSecurityMembershipUserClass and
      this.getExpr() = call.getArgumentForName("name")
    }

    override
    string getSinkDescription() {
      result = "the $@ parameter in $@"
    }
  ...
  class X509CertificateDataSink extends Sink {
    ...
    X509CertificateDataSink() {
      x509Creation.getTarget().getDeclaringType() instanceof SystemSecurityCryptographyX509CertificatesX509CertificateClass and
  ...
  class StringFormatSanitizer extends Sanitizer {
    StringFormatSanitizer() {
      this.getExpr() = any(SystemStringClass s).getFormatMethod().getACall().getArgumentForName("format")
    }
  }
  ...
  class StringReplaceSanitizer extends Sanitizer {
    StringReplaceSanitizer() {
      exists(SystemStringClass s, Call c |
        c = s.getReplaceMethod().getACall() |
  ...
  class ToStringSanitizer extends Sanitizer {
    ToStringSanitizer() {
      this.getExpr() = any(Call c | c.getTarget().hasName("ToString"))
    }
  }
}
