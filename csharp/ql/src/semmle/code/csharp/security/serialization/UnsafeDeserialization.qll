  ...
  class UnsafeDeserializerSink extends UnsafeDeserializationSink {
    UnsafeDeserializerSink () {
      exists(Call c | 
        ...
        and c.getTarget() instanceof UnsafeDeserializer)
    }
  }
  ...
  class JavaScriptSerializerSink extends UnsafeDeserializationSink {
    JavaScriptSerializerSink () {
      exists(ObjectCreation oc| 
        oc.getTarget().getDeclaringType().hasQualifiedName("System.Web.Script.Serialization.JavaScriptSerializer")
        and oc.getTarget().getNumberOfParameters() > 0
        and exists(MethodCall mc | 
          (mc.getTarget().hasQualifiedName("System.Web.Script.Serialization.JavaScriptSerializer", "Deserialize") or 
           mc.getTarget().hasQualifiedName("System.Web.Script.Serialization.JavaScriptSerializer", "DeserializeObject"))
           and this.asExpr() = mc.getAnArgument()
           and DataFlow::localFlow(DataFlow::exprNode(oc), DataFlow::exprNode(mc.getQualifier())))
      ) 
    }
  }
}
