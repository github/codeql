...
class XmlSerializableType extends SerializableType {
  XmlSerializableType() {
    this.isPublic() and not this instanceof BinarySerializableType and
  ...
  override Callable getADeserializationCallback() {
    result = any(Constructor c | c.getDeclaringType() = this and
      c.getNumberOfParameters() = 0
    ) or
    result = any(Setter s | s.getDeclaringType() = this.getABaseType*() and
      s.isPublic()
    )
  }
  ...
  override Field getASerializedField() {
    ...
    this.getADeserializationCallback().(Setter) = result.getAnAssignedValue().getEnclosingCallable() or
