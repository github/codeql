...
class SystemReflectionClass extends Class {
  SystemReflectionClass() {
    this.getNamespace() instanceof SystemReflectionNamespace
  }
}

/** The `System.Reflection.MethodBase` class. */
class SystemReflectionMethodBaseClass extends SystemReflectionClass {
  SystemReflectionMethodBaseClass() {
    this.hasName("MethodBase")
  }
  ...
  Method getInvokeMethod1() {
    ...
    and
    result.hasName("Invoke")
    and
    result.getNumberOfParameters() = 5
    and
    result.getParameter(0).getType() instanceof ObjectType
    and
    result.getParameter(3).getType().(ArrayType).getElementType() instanceof ObjectType
    and
  ...
  Method getInvokeMethod2() {
    ...
    result.getParameter(0).getType() instanceof ObjectType
    and
    result.getParameter(1).getType().(ArrayType).getElementType() instanceof ObjectType
    and
    result.getReturnType() instanceof ObjectType
  }
}
