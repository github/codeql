...
module SystemDataEntity {
  ...
  class Database extends Class {
    Database() {
      this.hasName("Database")
    }

    /** Gets the `ExecuteSqlCommand` method. */
    csharp::Method getExecuteSqlCommandMethod() {
      result = this.getAMethod("ExecuteSqlCommand")
    }
    ...
    csharp::Method getSqlQueryMethod() {
      result = this.getAMethod("SqlQuery")
    }
  }
  ...
  class DbSet extends Class {
    ...
    csharp::Type getDbSetType() {
      result = this.(csharp::ConstructedType).getTypeArgument(0)
    }
  }
...
module SystemDataEntityInfrastructure {
  ...
  class DbRawSqlQuery extends Class {
    DbRawSqlQuery() {
      this.getABaseType*().getSourceDeclaration().(csharp::UnboundGenericClass).getNameWithoutBrackets() = "DbRawSqlQuery"
    }
  }
}
