...
predicate likelyNonRepeatableSequence(IEnumerableSequence seq)
{
  ...
  exists(Expr src | src = sequenceSource(seq) |
    ...
    exists(MethodCall mc |
      mc = src
      and isIEnumerableType(mc.getTarget().getReturnType())
      and exists(MethodCall readlineCall |
        readlineCall.getTarget().hasName("ReadLine")
        and readlineCall.getEnclosingCallable() = mc.getTarget()
      )
    )

    // A call to Select(...) that instantiates new objects.
    or exists(SelectCall sc |
      sc = src
      and sc.getFunctionExpr().getExpressionBody() instanceof ObjectCreation
    )
  )
}
...
{
  exists(ForeachStmt fes |
    va = fes.getIterableExpr()
  )
  or exists(MethodCall mc |
    va = mc.getArgument(0)
    and mc.getTarget() instanceof ExtensionMethod
  )
}
...
Expr sequenceSource(IEnumerableSequence seq)
{
  ...
  or exists(Assignment a | a.getLValue() = seq.getAnAccess() and result = a.getRValue())
}

from IEnumerableSequence seq, VariableAccess va
where
  likelyNonRepeatableSequence(seq)
  and va = seq.getAnAccess()
  and potentiallyConsumingAccess(va)
  and count(VariableAccess x | x = seq.getAnAccess() and potentiallyConsumingAccess(x)) > 1
select seq, "This enumerable sequence may not be repeatable, but is potentially consumed multiple times $@.",
  va, "here"
